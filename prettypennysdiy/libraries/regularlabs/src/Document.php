...
class Document
{
	...
	public static function isClient($identifier)
	{
		...
		if (JVERSION < 3.7)
		{
			return $identifier == 'administrator' ? JFactory::getApplication()->isAdmin() : JFactory::getApplication()->isSite();
		}

		return JFactory::getApplication()->isClient($identifier);
	}
	...
	private static function scriptOptionsLegacy($options = [], $name = '')
	{
		...
		$options = array_map(function ($option_key, $option_value) {
			if ( ! is_bool($option_value) && ! is_int($option_value))
			{
		...
		$script = 'var ' . $key . '_options = {' . implode(',', $options) . '};';
