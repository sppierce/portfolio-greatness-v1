...
class AdminAttendeesController extends JControllerLegacy
{
	...
	function overview()
	{
		...
		else
		{
			list($year, $month, $day) = JEVHelper::getYMD();
                        ...
                        $repeat = $this->dataModel->queryModel->listEventsById (intval($rp_id), JFactory::getApplication()->isAdmin(), "icaldb");                          
		...
		if ($search)
		{
			...
			if ($rsvpdata->template != "")
			{
				...
				if (is_int($xmlfile) || file_exists($xmlfile))
				{
					...
					foreach ($tempparams as $param)
					{
						...
						{
                                                    // do we need to search amongst guest values - which would be an array 
                                                    if (isset($param["peruser"]) && $param["peruser"]>0) {
							$fieldsearch  = " OR atdees.params LIKE '%\"".$param["name"]. "\":[\"$search%'";
                                                    }
                                                    // or just the primary attendee
                                                    else {
                                                        $fieldsearch  = " OR atdees.params LIKE '%\"".$param["name"]. "\":\"$search%'";
                                                    }
			...
			$where[] = "(atdees.email_address LIKE '%$search%' OR ju.name LIKE '%$search%' OR ju.username LIKE '%$search%' OR ju.email LIKE '%$search%'  OR atdees.id = '$search' $fieldsearch)";
		}
	...
	function export()
	{
		...
		else
		{
			list($year, $month, $day) = JEVHelper::getYMD();
                        // include unpublished events
                        $repeat = $this->dataModel->queryModel->listEventsById (intval($rp_id), true, "icaldb");                          
			//$repeatdata = $this->dataModel->getEventData(intval($rp_id), "icaldb", $year, $month, $day);
			//if ($repeatdata && isset($repeatdata["row"]))
			//	$repeat = $repeatdata["row"];
		}
	...
	function message()
	{
		...
		$bcc= JRequest::getString("messagebcc", "");
                $replyto= JRequest::getString("replyto", null);
                if ($replyto==""){
                    $replyto = null;
                }
		...
		// Find the first repeat - including unpublished events
		$vevent = $this->dataModel->queryModel->getEventById($rsvpdata->ev_id, true, "icaldb");
		if ($rp_id == 0)
		{
			$repeat = $vevent->getFirstRepeat();
		...
		else
		{
			list($year, $month, $day) = JEVHelper::getYMD();
                        // include unpublished events
                        $repeat = $this->dataModel->queryModel->listEventsById (intval($rp_id), 1, "icaldb");                          
			//$repeatdata = $this->dataModel->getEventData(intval($rp_id), "icaldb", $year, $month, $day);
			//if ($repeatdata && isset($repeatdata["row"]))
			//	$repeat = $repeatdata["row"];
		}
		...
		if ($message != "" && $subject != "")
		{
			...
			foreach ($attendees as $attendee)
			{
				...
				$onWaitingList = false;
				$success = $this->notifyUser($rsvpdata, $row, $user, $name, $username, $attendee, 'SPECIAL_MESSAGE', $onWaitingList, false, $subject, $message, false, $replyto);
                                $sent += $success? 1 : 0;
			}
			...
			if (isset($attendee)){
				...
				$onWaitingList = false;
				$success = $this->notifyUser($rsvpdata, $row, $creator, $creator->name, "", $attendee, 'SPECIAL_MESSAGE', $onWaitingList, false, $subject, $message, false, $replyto);
                                $sent += $success? 1 : 0;
			}
			
			$this->setRedirect("index.php?option=com_rsvppro&task=attendees.overview&atd_id[]=" .$atd_id."|".$rp_id . "&repeating=" . $repeating . "&Itemid=" . JRequest::getInt("Itemid", 0), JText::sprintf("RSVP_MESSAGES_SENT", $sent));
			return;
		}
	...
	function notwaiting()
	{
		...
		$db = JFactory::getDBO();
                $now = JevDate::getDate("+0 seconds");
		$db->setQuery("UPDATE #__jev_attendees set waiting=0, modified=".$db->Quote($now->toMySql())." where id=" . $atdee);
		$db->execute();
	...
	function waiting()
	{
		...
		$db = JFactory::getDBO();
                $now = JevDate::getDate("+0 seconds");
		$db->setQuery("UPDATE #__jev_attendees set waiting=1, modified=".$db->Quote($now->toMySql())." where id=" . $atdee);
		$db->execute();
	...
	function confirm()
	{
		...
		$db = JFactory::getDBO();
                $now = JevDate::getDate("+0 seconds");
                $db->setQuery("UPDATE #__jev_attendees set confirmed=1, modified=".$db->Quote($now->toMySql())." where id=" . $atdee);
		$db->execute();
	...
	function approve()
	{
		...
		$db = JFactory::getDBO();
                $now = JevDate::getDate("+0 seconds");
		$db->setQuery("UPDATE #__jev_attendees set attendstate=1, modified=".$db->Quote($now->toMySql())." where id=" . $atdee);
		$db->execute();
	...
	{
		$atd_id = JRequest::getVar("atd_id", array(), "method", "array");
		if (!isset($atd_id[0]) || strpos($atd_id[0], "|") === false)
		{
			JError::raiseError("403", JText::_("RSVP_MISSING_ATDID"));
		}
		...
		$atdee = $cid[0];
		$notes = JRequest::getVar("notes", array(), "method","array");
		...
		$db = JFactory::getDBO();
                $now = JevDate::getDate("+0 seconds");
		$db->setQuery("UPDATE #__jev_attendees set modified=".$db->Quote($now->toMySql()).", notes=".$db->quote($notes)." where id=" . $atdee);
		$db->execute();

		$this->setRedirect("index.php?option=com_rsvppro&task=attendees.overview&atd_id[]=" . $atd_id[0] . "&repeating=" . $repeating . "&Itemid=" . JRequest::getInt("Itemid", 0), JText::_("JEV_NOTES_UPDATED"));
	...
	function attend()
	{
		...
		$gc = JRequest::getInt("gc", 0, "method");
		...
		$db = JFactory::getDBO();
                $now = JevDate::getDate("+0 seconds");
		$db->setQuery("UPDATE #__jev_attendees set didattend=1 , modified=".$db->Quote($now->toMySql()).", notes=".$db->quote($notes).", guestattend =".$db->quote($guestatttend)." where id=" . $atdee);
		$db->execute();
	...
	function notattend()
	{
		...
		$db = JFactory::getDBO();
                $now = JevDate::getDate("+0 seconds");
		$db->setQuery("UPDATE #__jev_attendees set didattend=0, modified=".$db->Quote($now->toMySql()).", guestattend='', notes=".$db->quote($notes)."  where id=" . $atdee);
		$db->execute();
	...
	function delete()
	{
		...
		if ($vevent && $rsvpdata->allowreminders)
		{
			...
			JFactory::getApplication()->enqueueMessage(JText::_("JEV_REMINDER_CANCELLED"));
		}

                // clean up update actions in plugins
                if ($repeat){
                    $rsvpparams = $this->getRSVPParmeters($rsvpdata, $repeat);
                    foreach ($cid as $jevattend_id)
                    {
                            $sql = "SELECT * FROM #__jev_attendees WHERE id=" . $jevattend_id;
                            $db = JFactory::getDBO();
                            ...
                            $this->postUpdateActions($rsvpdata, $repeat, $attendee,$attendee->waiting, $rsvpparams);
                    }
                }
	...
	function save()
	{
		...
		$guestcount = JRequest::getInt("guestcount", 1);
                $guestcountchanged = intval($attendee->guestcount) !==  $guestcount;
		...
		$attendee->waiting = 0;
                // only make them waiting if attending, pending approval or outstanding balance
		if ($this->params->get("capacity", 0) && $rsvpdata->capacity > 0 && in_array($attendee->attendstate, array(1,3,4)))
		{
			if ($attendeeCount >= intval($rsvpdata->capacity) && $guestcountchanged)
			{
				$attendee->waiting = 1;
			}
		}
                ...
                if (!isset($attendee->outstandingBalances))
                {
                        ...
                        if (isset($attendee->params))
                        {
                                ...
                                if (is_int($xmlfile) || file_exists($xmlfile))
                                {
                                        ...
                                        // Any transactions to update
                                        $db = JFactory::getDbo();
                                        $db->setQuery("SELECT transaction_id FROM #__jev_rsvp_transactions where attendee_id= " . $attendee->id. " AND paymentstate=0 and gateway='manual' ORDER BY transaction_date desc limit 1 ");
                                        $transaction_id = $db->loadResult();
                                        if ($transaction_id && isset($feesAndBalances['feebalance'])){
                                            $transaction = new rsvpTransaction();
                                            $transaction->load($transaction_id);
                                            $transaction->amount = $feesAndBalances['feebalance'];
                                            $transaction->store();
                                        }
                                }
                        }
		...
		// auto remind attendees - not those on waiting list and only if actually attending
		if ($this->params->get("autoremind", 0) == 1 && !$attendee->waiting && in_array($attendee->attendstate, array(1,3,4)) && ($rsvpdata->allowreminders || $this->params->get("forceautoremind", 0)))
		{
			// create reminder
			$datamodel = new JEventsDataModel();
			if ($rp_id > 0)
		...
		if ($this->params->get("notifyuser", 0) && $atdee ==0 )
		{
			$this->notifyUser($rsvpdata, $row, $user, $name, $username, $attendee, 'ack', false);
                ...
                if ($attendee->attendstate == 1 || $attendee->attendstate == 3)
                {
                        ...
                        if ($this->params->get("notifycreator", 0) && $this->params->get("notifycreator", 0)!=3) {
                                ...
                                $this->notifyCreator($rsvpdata, $row, $name, $username, $attendee, false, $attendee->waiting);
                        }
                }
	...
	{     
		...
		//$attendee = $db->loadObject();
               ...
               $now = JevDate::getDate("+0 seconds");
		$db->setQuery("UPDATE #__jev_attendees set attendstate=$newstate, modified=".$db->Quote($now->toMySql())." where id=" . $atdee);
		$db->execute();

                // Manage auto reminders
		if ($this->params->get("autoremind", 0) == 1 && $eventrow)
		{
			...
			if ($rsvpdata->allowreminders || $this->params->get("forceautoremind", 0) == 1)
			{
                            ...
                            if ($newstate==1){
				// create reminder
				$this->jevrreminders->remindUser($rsvpdata, $eventrow, $user, $attendee->email_address);
				$mainframe = JFactory::getApplication();
				$mainframe->enqueueMessage(JText::_("JEV_REMINDER_CONFIRMED"));
                            }
                            else if ($newstate==0 || $newstate==2){
				// remove reminder
				$this->jevrreminders->unremindUser($rsvpdata, $eventrow, $user, $attendee->email_address);
				$mainframe = JFactory::getApplication();
                ...
                // clean up update actions in plugins
                if ($eventrow){
                    $rsvpparams = $this->getRSVPParmeters($rsvpdata, $eventrow);
                    foreach ($cid as $jevattend_id)
                    {
                            $sql = "SELECT * FROM #__jev_attendees WHERE id=" . $jevattend_id;
                            $db = JFactory::getDBO();
                            ...
                            $this->postUpdateActions($rsvpdata, $eventrow, $attendee,$attendee->waiting, $rsvpparams);
                    }
                }
	 ...
	 */
	private function notifyUser($rsvpdata, & $row, $user, $name, $username, $attendee = null, $messagetype = 'ack', $onWaitingList = false, $transaction = false, $subject = "", $message = "", $speciallink=false, $replyto=null) {
		if (isset($this->event) && !isset($this->helper->event)){
			$this->helper->event = $this->event;
		}
		return $this->helper->notifyUser($rsvpdata,  $row, $user, $name, $username, $attendee, $messagetype, $onWaitingList, $transaction, $subject, $message, $speciallink, $replyto);
	}
		...
		if (!$repeat)
		{
			...
			$vevent = $this->dataModel->queryModel->getEventById($rsvpdata->ev_id, true, "icaldb");
                        ...
                        if (!$vevent	) {
                                $lang = JFactory::getLanguage();
                                $lang->load(RSVP_COM_COMPONENT, JPATH_ADMINISTRATOR);
                                JError::raiseError(403, JText::_("RSVP_NOT_AUTHORISED") . " yyy ");
                        }
		...
		if ($rsvpdata->template != "")
		{
			...
			if (is_int($xmlfile) || file_exists($xmlfile))
			{
				// Data now comes in via xmlfile field array
                                //$jinput = JFactory::getApplication()->input;
                                //$paramsnew = $jinput->post->get("xmlfile", $jinput->post->get("params", array(), "array"), "array");
                                //$filter = JFilterInput::getInstance();
                                //$paramsnew = $filter->clean($paramsnew, "default");
				...
				$params = $jregistry->toString();
                                ...
                                $params = $safeHtmlFilter->clean($params, "html");
