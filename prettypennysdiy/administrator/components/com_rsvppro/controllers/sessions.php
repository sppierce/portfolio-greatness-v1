...
class AdminSessionsController extends JControllerLegacy {
	...
	function overview( )
	{
		...
		$jevparams =  JComponentHelper::getParams(JEV_COM_COMPONENT);
                $rsvpparams = JComponentHelper::getParams(RSVP_COM_COMPONENT);

		$search		= $mainframe->getUserStateFromRequest( "search{".RSVP_COM_COMPONENT."}", 'search', '' );
		$search		= $db->escape( trim( strtolower( $search ) ) );

		$after		= $mainframe->getUserStateFromRequest( "after{".RSVP_COM_COMPONENT."}", 'after', '' );
		$after		= $db->escape( trim( strtolower( $after ) ) );
                if ($after){
                    $inputdateformat = $jevparams->get("com_editdateformat", "d.m.Y");
                    $after = DateTime::createFromFormat($inputdateformat, $after);
		...
		$searchattendees = $db->escape(trim(strtolower($searchattendees)));
                ...
                $searchdidattend  = $mainframe->getUserStateFromRequest("atsearchdidattend{" . RSVP_COM_COMPONENT . "}", 'searchdidattend', 0);
                $searchdidattend  = intval($searchdidattend);
                $postinput = new JInput($_POST);
                if (!$searchattendees || ($postinput->get("task","","cmd")=="sessions.list" && $postinput->get("searchdidattend",0,"int")==0)){
                    $searchdidattend  = 0;
                    $mainframe->setUserState("atsearchdidattend{" . RSVP_COM_COMPONENT . "}", $searchdidattend);
                }
		...
		{
                        if ($searchattendees=="*"){
                            $where[] = "atdees.id IS NOT NULL ";
                            //$where[] = "atdees.attendstate=1 ";
                        }
                        else {
                            $searchallfields = $rsvpparams->get("searchallfields",0);
                            if ($searchallfields ){
                                $where[] = "(atdees.params LIKE '%$searchattendees%' OR atdees.email_address LIKE '%$searchattendees%' OR jua.username LIKE '%$searchattendees%' OR jua.email LIKE '%$searchattendees%'  OR atdees.id = '$searchattendees' )";                                
                            }
                            else {
		...
		if ($searchdidattend && $searchattendees)
		{
                    $where[] = "atdees.didattend=1";
                }
		...
		// category filter!
			...
			else
			{
                ...
                if ($after) {
                    $where[] = "\n (rpt.startrepeat >".$db->quote($after)." OR atd.regclose>".$db->quote($after).")";
                }
		...
		if ($nonrepeating){
			if ($hidepast){
				...
				$where[] = "\n det.dtstart>'".$datenow->toUnix()."'";
                        ...
                        if ($rsvpparams->get("attendance",0) && $rsvpparams->get("invites",0)){
        			$where[] = "((atd.allrepeats=1 and atd.allowregistration>0 ) OR (atd.invites=1 AND atd.allinvites=1))";                                                        
                        }
                        else if ($rsvpparams->get("invites",0)){
                            $where[] = "(atd.invites=1 AND atd.allinvites=1)";
                        }
                        else if ($rsvpparams->get("attendance",0)){
			...
			//$where[] = "(atd.allrepeats=0 OR atd.allinvites=0) AND (atd.allowregistration>0 OR atd.invites=1)";
        			...
        			$where[] = "(atd.allrepeats=0 OR atd.allinvites=0) AND (atd.allowregistration>0 OR atd.invites=1)";
                            ...
                            $where[] = "atd.allinvites=0 AND atd.invites=1";
                        }
                        else if ($rsvpparams->get("attendance",0)){
		...
		$this->view->assign('searchdidattend',$searchdidattend);
