	...
	function store( $updateNulls=false ) {
		...
		if (count($oldids)>0){
			...
			$success |= parent::store($updateNulls);
                ...
                // Do we need to copy any translations?
		$copytranslations =  JRequest::getInt("copytranslations",0);
		if ($copytranslations && JRequest::getInt("oldid",0) > 0){
                    $oldtemplateid = JRequest::getInt("oldid",0);
                    $newtemplateid = $this->id;
                    foreach ($this->fields as $field) {
                        if (isset($field->_old_field_id) && $field->_old_field_id>0){
                            $this->_db->setQuery("SELECT * from #__jev_rsvp_fields_translation where field_id=".$field->_old_field_id." AND template_id=".$oldtemplateid );
                            $oldtranslations = $this->_db->loadObjectList();
                            foreach ($oldtranslations as $oldtranslation){
                                $tableclass = "Table".ucfirst($field->type);
				if (!class_exists($tableclass) && file_exists(JPATH_COMPONENT_ADMINISTRATOR."/tables/field.".$field->type.".php")){
					include_once(JPATH_COMPONENT_ADMINISTRATOR."/tables/field.".$field->type.".php");
					$fieldtranslation = new $tableclass();
				}
                                if (class_exists($tableclass)){
                                    $fieldtranslation = new $tableclass();
                                }
				else {
					include_once(JPATH_COMPONENT_ADMINISTRATOR."/tables/field.php");
					$fieldtranslation = new TableField();
				}

                                // Change from a normal field into a field translation (I'm being lazy!)
                                //$fieldtranslation->convertToTranslation();
                                //$fieldtranslation->load($oldtranslation->translation_id);
                                $this->_db->setQuery("SELECT * FROM #__jev_rsvp_fields_translation where translation_id=".$oldtranslation->translation_id);
                                $existingTranslation = $this->_db->loadObject();
                                if ($existingTranslation) {
                                    $vars = get_object_vars($existingTranslation);
                                    foreach ($vars as $k => $v){
                                        $fieldtranslation->$k = $v;
                                    }
                                    $fieldtranslation->translation_id=0;
                                    $fieldtranslation->field_id = $field->field_id;
                                    $fieldtranslation->template_id = $newtemplateid;                                   
                                    $fieldtranslation->storeTranslation();
                                }
                            }
                        }
                    }
                    // Now copy the template translation itself
                    $this->_db->setQuery("INSERT INTO #__jev_rsvp_template_translation (template_id, title, description, ticket, notes,params, language)  SELECT $newtemplateid, title, description, ticket, notes,params, language FROM #__jev_rsvp_template_translation");
                    $this->_db->execute();
                }                
