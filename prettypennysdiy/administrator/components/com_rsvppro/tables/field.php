	...
	public function bind($array, $ignore=array(), $fieldid="")
	{
		...
		foreach ($array as $key=>$data){
			switch ($key){
				...
				case 'ph':
					if (array_key_exists($fieldid,$data)){
						$this->placeholder = $data[$fieldid];
					}
					break;	
				case 'cn':
					if (array_key_exists($fieldid,$data)){
						$this->classname = $data[$fieldid];
					}
					break;	
				case 'rr':
					if (array_key_exists($fieldid,$data)){
        ...
        public function convertToTranslation() 
        {
		...
		if (!isset($fields)){
			...
			if (strpos($key, "_")!==0 && !isset($this->$key)){
                            $this->$key = isset($field->Default) ? $field->Default : null;
                        }
                }
