...
class RsvpHelper
{
	...
	static function requiredAnyAll($id, $field, $default = 0)
	{
		?>
		<div class="rsvpnotranslate">
		<div class="rsvplabel"><?php echo JText::_("RSVP_IS_REQUIRED"); ?></div>
		<div class="rsvpinputs radio btn-group">
			<label for="rr1<?php echo $id; ?>" class="btn radio"><?php echo JText::_("RSVP_ALL_REQUIRED"); ?>
			<input type="radio" name="rr[<?php echo $id; ?>]"  id="rr1<?php echo $id; ?>" value="1" <?php
		if (($field && $field->required == 1) || (!$field && $default) )
		{
			echo 'checked="checked"';
		...
		?> />
			...
			<label for="rr2<?php echo $id; ?>" class="btn radio"><?php echo JText::_("RSVP_ANY_REQUIRED"); ?>
			<input type="radio" name="rr[<?php echo $id; ?>]"  id="rr2<?php echo $id; ?>" value="2" <?php
		if (($field && $field->required == 2) || (!$field && $default) )
		{
			echo 'checked="checked"';
		}
		?> />
	...
	static function placeholder($id, $field)
	{
		?>
		<div class="rsvpnotranslate">
		<div class="rsvplabel"><label for="ph<?php echo $id; ?>"><?php echo JText::_("RSVP_PLACEHOLDER"); ?></label></div>
		<div class="rsvpinputs">
			<input type="text" name="ph[<?php echo $id; ?>]" id="ph<?php echo $id; ?>" value="<?php echo $field ? $field->placeholder : ''; ?>" size="40" maxlength="255" />
		</div>
		<div class="rsvpclear"></div>
		</div>
		<?php
	...
	static function className($id, $field)
	{
		$params = JComponentHelper::getParams("com_rsvppro");
		if ($params->get("showclassname",0)) {
		?>
		<div class="rsvpnotranslate">
		<div class="rsvplabel"><?php echo JText::_("RSVP_CLASSNAME"); ?></div>
		<div class="rsvpinputs">
			<input type="text" name="cn[<?php echo $id; ?>]" id="cn<?php echo $id; ?>" value="<?php echo $field ? $field->classname : ''; ?>" size="40" maxlength="255" />
		</div>
		<div class="rsvpclear"></div>
		</div>
		<?php
                    ...
                    ?>
                <input type="hidden" name="cn[<?php echo $id; ?>]" id="cn<?php echo $id; ?>" value="<?php echo $field ? $field->classname : ''; ?>" size="40" maxlength="255" />
                    <?php
                }
	...
	static function hidden($id, $field, $name)
	{
		...
		<input type="hidden" name="ordering[<?php echo $id; ?>]" id="ordering<?php echo $id; ?>" value="<?php echo $field ? $field->ordering : 0; ?>" />
                <textarea style="display:none;" name="defaultvalue[<?php echo $id; ?>]" id="defaultvalue<?php echo $id; ?>" ><?php echo ($field && is_string($field->defaultvalue)) ? $field->defaultvalue : 0 ?></textarea>
		<?php

	}

                static function textField($id, $name, $field, $label, $tooltip,  $fieldattribute, $fielddefault = "", $attributes = "") {
                    $value = $fielddefault;
                    if (isset($field) && !isset($field->$fieldattribute)){
                        if (isset($field->params) && $field->params!=""){
                            $params = json_decode($field->params);
                            if (isset($params->$fieldattribute)){
                                $value = $params->$fieldattribute;
                            }
                        }
                    }
                    ?>
        ...
        <?php echo $label; ?>
        <?php RsvpHelper::fieldTooltip($tooltip); ?>
            </label>
        </div>
        <div class="rsvpinputs">
            <input type="text" name="<?php echo $name; ?>" id="<?php echo $id; ?>" <?php echo $attributes; ?>  
        ...
        <div class="rsvpclear"></div>
                ...
                static function textArea($id, $name, $field, $label, $tooltip,  $fieldattribute, $fielddefault = "", $attributes = "", $rows=5, $cols=60) {
                    $value = $fielddefault;
                    if (isset($field) && !isset($field->$fieldattribute)){
                        if (isset($field->params) && $field->params!=""){
                            $params = json_decode($field->params);
        ...
        <div class="rsvpinputs">
            <textarea 
            name="<?php echo $name; ?>" id="<?php echo $id; ?>" <?php echo $attributes; ?>  
             rows="<?php echo $rows;?>" cols="<?php echo $cols;?>" style="width:auto"
        ...
        static function fieldTooltip($fieldtooltip = false) {
            ...
            if ($fieldtooltip) {
                if (strpos($fieldtooltip, "JEVR_") === 0) {
                    $fieldtooltip = JText::_($fieldtooltip);
                }
	...
	static function peruser($id, $field)
	{
		...
		<div class="rsvpinputs">
			...
			$options[] = JHtml::_('select.option', 2, JText::_("RSVP_PRIMARY_GUESTS_ONLY"));
                        ...
                        JText::script('JEV_CANNOT_HAVE_FIELD_OPTION_CAPACITIES_WITH_GUESTS');
			// build the html select list
			echo JHtml::_('select.genericlist', $options, $fieldname, 'class="inputbox" size="1" onchange="rsvpforms.fieldOptionCapacityWarning(\''.$id.'\');return false;" ', 'value', 'text', $value);
			?>
		</div>
		<div class="rsvpclear"></div>
		</div>
...
SCRIPT;
	...
	static function phpMoneyFormat($amount)
	{
		...
		if ($digits >= 0)
		{
			...
			*/
			...
			//$formattedText .= abs(number_format($amount, $digits, $decimal, $separator));
		}
		else
		{
	...
	static function conditional($id, $field)
	{
		...
		<div class="rsvpinputs"   >
			...
			<div class="rsvpconditionalselector"  <?php echo $style;?>>
				...
				<div class="rsvpinputs">
                                        <?php // we must store this value in a hidden field since javascript will use it to populate the correct selected option ?>
	...
	public static function tooltipText($title = '', $content = '', $translate = 1, $escape = 1)
	{
        ...
        public static function translateTemplateParams($templateid, $params)
        {                                        
            // Do we have a translation for each of the messages
	        $db = JFactory::getDbo();
	        $jinput = JFactory::getApplication()->input;
	        $lang = JFactory::getLanguage();
            $lang = $jinput->getString("lang", $lang->getTag());
            $db->setQuery("SELECT params FROM #__jev_rsvp_template_translation where template_id = ".intval($templateid) . " AND language = ". $db->quote($lang));
            $tempdata = $db->loadObject();
            if ($tempdata){
                    $tempdata = json_decode($tempdata->params);
                    foreach (get_object_vars($params) as $key => $val) {
                            if (isset($tempdata->$key)){
                                    $params->$key = $tempdata->$key;
                            }
                    }
            }
            return $params;            
