...
SCRIPT;
		...
		foreach ($fieldgroup as $id => & $field)
		{
			...
			{
				$html [] = '<tr class="param' . $field->fieldname . ' type1param fieldtype' . $field->attribute('type') . ' ' . $conditionalclass . ' ' . $field->attribute('classname') . '">';
			}
			// If the node only applies to second and subsequent attendees then hide initially
			else if ($field->attribute("peruser", 0) == 2)
			{
				$html [] = '<tr class="param' . $field->fieldname . ' type2param  fieldtype' . $field->attribute('type') . ' ' . $field->attribute('classname') . ' ">';
			}
			else if ($field->attribute("peruser", 0) == 1)
			{
				$html [] = '<tr class="param' . $field->fieldname . ' type1param  fieldtype' . $field->attribute('type') . ' ' . $field->attribute('classname') . ' ">';
			}
			else
			{
				$html [] = '<tr class="param' . $field->fieldname . ' type0param ' . $conditionalclass . ' fieldtype' . $field->attribute('type') . ' ' . $field->attribute('classname') . ' ">';
			}
			...
			{		
				...
				JevHtmlBootstrap::popover('.hasjevtip' , array("trigger"=>"hover focus", "placement"=>"top", "container"=>"#jevents", "delay"=> array( "show"=> 150, "hide"=> 150 )));
	...
	public function renderToBasicArray($group = 'xmlfile', $attendee = false)
	{
		...
		$cacheid .= isset($this->outstandingBalances) ? serialize($this->outstandingBalances) : "false";
                $cacheid .= isset($this->rsvpdata) ? $this->rsvpdata->id. "_".$this->rsvpdata->ev_id : "false";
		...
		foreach ($fieldgroup as & $field)
		{
			...
			$result['validto'] = $field->attribute('validto');
                        $result['classname'] = $field->attribute('classname', '');
			...
			if (!$accessible)
			{
				$result['value'] = "";
                        ...
                        $result["nativevalue"] = $result["value"];
			...
			if ($attendee && method_exists($field, "convertValue") && $accessible)
			{
				if ($field->attribute('peruser') > 0 && $attendee->guestcount > 1)
				{
					...
					else
					{
						...
						else
						{
							...
							JError::RaiseWarning(300, "We have a problem with the values of " . $field->name . " : ". $field->attribute('label'));
							if ($attendee)
							{
				...
				else
				{
					...
					{
						if ($field->attribute('type') == 'jevrbalance' || $field->attribute('type') == 'jevrcbavatar' || $field->attribute('type') == 'jevrcbfield' || $field->attribute('type') == 'jevrcoupon'  || $field->attribute('type') == 'jevrprofilefield' || $field->attribute('type') == 'jevrinstruction')
						{
		...
		if (is_callable("gzcompress") && $cacheid != "falsefalsefalse")
		{
...
//			if (method_exists($field, "convertValue"))				$result['value'] = $field->convertValue($result['value']);
	...
	public function outstandingBalance(&$attendee)
	{
		...
		if ($attendee && $attendee->id > 0)
		{
			...
			if ($feebalance > 0.001)
			{
				$db->setQuery("UPDATE #__jev_attendees set attendstate=4, modified=".$db->Quote($now->toMySql())." WHERE id=" . $attendee->id . " AND attendstate=1");
				$db->execute();
			}
			else
			{
				$db->setQuery("UPDATE #__jev_attendees set attendstate=1, modified=".$db->Quote($now->toMySql())." WHERE id=" . $attendee->id . " AND attendstate=4");
				$db->execute();
			}
		}
...
<form action="' . JRoute::_('index.php?option=com_rsvppro&task=accounts.paymentpage&gateway=' . $gateway . '&Itemid=' . $Itemid . $tmpl) . '" method="POST" name="jevrpaymentform'.$pf.'">
	...
	public function getTicket($attendee, $rsvpdata, $event)
	{
		...
		if (count($matches) == 2)
		{
			...
			if (isset($attendee->outstandingBalances) && isset($attendee->outstandingBalances['hasfees']) && $attendee->outstandingBalances['hasfees'])
			{
				...
				$ticketintro = str_ireplace("{BALANCE}", "", $ticketintro);
                                $ticketintro = str_ireplace("{RAWBALANCE}", "", $ticketintro);
		...
		if (count($matches) == 2)
		{
			...
			if (isset($attendee->outstandingBalances) && isset($attendee->outstandingBalances['hasfees']) && $attendee->outstandingBalances['hasfees'])
			{
				...
				$ticketfooter = str_ireplace("{BALANCE}", RsvpHelper::phpMoneyFormat($attendee->outstandingBalances['feebalance']), $ticketfooter);
                                $ticketfooter = str_ireplace("{RAWBALANCE}", $attendee->outstandingBalances['feebalance'], $ticketfooter);
				...
				$ticketfooter = str_ireplace("{BALANCE}", "", $ticketfooter);
                                $ticketfooter = str_ireplace("{RAWBALANCE}", "", $ticketfooter);
		...
		for ($i = 0; $i < $attendee->guestcount; $i++)
		{
			...
			$tickettemplate = str_replace("{CATEGORY}", $event->catname(),$tickettemplate);
                        ...
                        $tickettemplate = str_replace("{USERNAME}", $username ,$tickettemplate);
			...
			if (isset($attendee->outstandingBalances) && isset($attendee->outstandingBalances['hasfees']) && $attendee->outstandingBalances['hasfees'])
			{
				...
				$tickettemplate = str_ireplace("{BALANCE}", RsvpHelper::phpMoneyFormat($attendee->outstandingBalances['feebalance']), $tickettemplate);
                                $tickettemplate = str_ireplace("{RAWBALANCE}", $attendee->outstandingBalances['feebalance'], $tickettemplate);
				...
				$tickettemplate = str_ireplace("{BALANCE}", "", $tickettemplate);
                                $tickettemplate = str_ireplace("{RAWBALANCE}", "", $tickettemplate);
			...
			$tickettemplate = str_ireplace("{BARCODE}", "<img src='" . JURI::root() . "components/com_rsvppro/assets/images/image.php?bc=" . $code . "' alt='Barcode'/>", $tickettemplate);
                        ...
                        if ($rsvpparams->get("rewriteqr",0)){
                            $tickettemplate = str_ireplace("{QR}", '<img src="' . JURI::root() . "components/com_rsvppro/assets/images/qrimage$code.png" .'" alt="QR" />', $tickettemplate);                            
                        }
                        else {
			...
			*/
			...
			foreach ($data as $fieldname => $field)
			{
				if ($field['peruser'] <= 0 && $i > 0)
				{
					$value = "";
					$label = "";
                                        // OSLO MOD - allows primary fields to be output for all guests
                                        //$value = $field["value"];
                                        //$label = $field["label"];
					...
					if (isset($field["price"])) {
						if ((is_array($field["price"]) && count($field["price"]) == $attendee->guestcount) || $field['peruser']==0 )
						{
							...
							// radio field
								...
								if (isset($field["price"][0]) && floatval(RsvpHelper::phpMoneyFormat($field["price"][0]))!=0){
			...
			$ticket .= $tickettemplate;
                        // OSLO MOD - outputs intro after every 2 guests
                        //if ($i%2){
                        //    $ticket .= $ticketintro;
                        //}
		}
	...
	public function calculateRowContributionsToCapacity(& $result, $type, & $attendee)
	{
		...
		if (!$attendee)
		{
			...
			{
				$sql = "SELECT params, guestcount, waiting  FROM #__jev_attendees as a  WHERE a.at_id=" . $rsvpdata->id . " AND attendstate=1";
			}
			else
			{
				$sql = "SELECT params, guestcount, waiting  FROM #__jev_attendees as a  WHERE a.at_id=" . $rsvpdata->id . " AND (attendstate=1 || attendstate=4)";
			}
		...
		$field = $this->loadFieldType($type);
                ...
                //Must reset reducetotalcapacity field incase this method has been called mulitple times e.g. from renderToBasicArray
                $result['reducetotalcapacity'] = 0;
                $result['waitingreducetotalcapacity'] = 0;
		...
		if ($result['capacity'] > 0 || method_exists($field, "totalCapacityContribution") || $rsvpdata->capacity > 0)
		{
			...
			for ($i = 0; $i < count($attendeeData); $i++)
			{
				...
				foreach ($pvalues as $pvalue)
				{
					...
					if ($pvalue > 0)
					{
						...
						$count+= $pvalue;
                                                $waitingcount += $attendeeData[$i]->waiting ? $pvalue : 0;
						...
						$result['reducetotalcapacity'] += $result['reducevaluefortotalcapacity'];
                                                $result['waitingreducetotalcapacity'] += $attendeeData[$i]->waiting ? $result['reducevaluefortotalcapacity'] : 0 ;
			...
			$result['capacitycount'] = $count;
                        $result['waitingcapacitycount'] = $waitingcount;
		}
	...
	protected function convertTemplateToXML($template)
	{
		...
		$xml[] = '<fieldset name="xmlfile" addfieldpath="/administrator/components/com_rsvppro/customfields/" >';
                // The description field from the template is not picked up here.
		$xml[] = '<description>';
		$xml[] = '<![CDATA[';
