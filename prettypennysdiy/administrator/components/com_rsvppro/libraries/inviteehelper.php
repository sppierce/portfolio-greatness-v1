...
class RsvpInviteeHelper
{
		...
		if ($user->id == $row->created_by() || JEVHelper::isAdminUser($user) || JEVHelper::canDeleteEvent($row, $user))
		{
			...
			if ($jevattend_hiddeninitees)
			{
				$jevinvitees = JRequest::getVar("jevinvitee", array(), 'post', 'array');                                
                                // Max number of emails to send in one go
                                $maxsendinvites = $this->params->get("maxsendinvites", 30);
                                $invitesSent = 0;
                                $maxInvitesMessageDisplayed = false;
				...
				$keeplist = array();
                                ...
                                // which have been resent
                                $resentList = (array) json_decode(JFactory::getApplication()->getUserState("resentList", "{}"));
                                // If all have been resent then this is ANOTHER resend to reset the list
                                if (JRequest::getString("rsvp_email", "", "post") == "reemail"){
                                    $leftToSend = array_diff($jevinvitees, $resentList);
                                    if (count($leftToSend)==0){
                                        $resentList = array();
                                        JFactory::getApplication()->setUserState("resentList", json_encode($resentList));
                                    }
                                }
				...
				foreach ($invitees as $invitee)
				{
					...
					if (!$currentinvitee)
					{
						...
						$currentinvitee->iid = $currentinvitee->id;
                                                ...
                                                $this->recordRsvpActivity("rsvppro.invite", $row);
						...
						if (JRequest::getString("rsvp_email", "", "post") == "email" || JRequest::getString("rsvp_email", "", "post") == "reemail" || JRequest::getString("rsvp_email", "", "post") == "failed")
						{
							if ($iuser)
							{
                                                                if ($invitesSent >= $maxsendinvites){
                                                                    JFactory::getApplication()->enqueueMessage(JText::sprintf("JEV_MAX_INITES_EMAILED", JText::_("JEV_CLICK_TO_RESEND_FAILED_MESSAGES")));
                                                                    $maxInvitesMessageDisplayed = true;
                                                                    // we need to save the invitee record
                                                                    continue ;
                                                                }
						...
						if (JRequest::getString("rsvp_email", "", "post") == "reemail" || JRequest::getString("rsvp_email", "", "post") == "failed")
						{
							...
							if ($currentinvitee->attending)
							{
								continue;
                                                        ...
                                                        // already resent
                                                        if (in_array("rsvp_inv_".$invitee, $resentList)){
                                                            continue;
                                                        }
                                                        
                                                        if ($invitesSent >= $maxsendinvites ) {
                                                            if (!$maxInvitesMessageDisplayed) {
                                                                JFactory::getApplication()->enqueueMessage(JText::sprintf("JEV_MAX_INITES_EMAILED", JText::_("JEV_CLICK_TO_RESEND_FAILED_MESSAGES")));
                                                            }
                                                            $maxInvitesMessageDisplayed = true;                                                            
                                                            continue;
					...
					if (!$currentinvitee)
					{
						...
						if (JRequest::getString("rsvp_email", "", "post") == "email" || JRequest::getString("rsvp_email", "", "post") == "reemail" || JRequest::getString("rsvp_email", "", "post") == "failed")
						{
                                                        if ($invitesSent >= $maxsendinvites) {
                                                            ...
                                                            $maxInvitesMessageDisplayed = true;
                                                            // do not break since we need to save the invitees!
                                                            continue;
                                                        }
			...
			function parseMessage($message, $rsvpdata, & $row, $name, $requirelogin = false, $currentinvitee, $parsetype = "subject")
	{
		...
		else
		{
			...
			// direct attendance link
			$daregex = "#{DIRECTATTENDLINK}(.*?){/DIRECTATTENDLINK}#s";
			preg_match($daregex, $message, $damatches);
			$dnaregex = "#{DIRECTNOATTENDLINK}(.*?){/DIRECTNOATTENDLINK}#s";
			preg_match($dnaregex, $message, $dnamatches);
			
			if (count($matches) == 2 || count($damatches) == 2  || count($dnamatches) == 2)
			{
				$Itemid = JRequest::getInt("Itemid");
				list($year, $month, $day) = JEVHelper::getYMD();
				// Do NOT use SEF because not consistent between frontend and backend!
				...
				// direct attendance link
                                ...
                                if (isset($currentinvitee->email_address) && $currentinvitee->email_address!=""){
                                    $daemailaddress = $currentinvitee->email_address ;
                                }
                                else if (isset($currentinvitee->email) && $currentinvitee->email!=""){
                                    $daemailaddress = $currentinvitee->email ;
                                }
                                else if (isset($currentinvitee->user_id) && $currentinvitee->user_id){
                                    $daemailaddress = JFactory::getUser($currentinvitee->user_id)->email ;
                                }
				$params = JComponentHelper::getParams("com_rsvppro");
				$dacode = base64_encode($daemailaddress . ":" . md5("directattend".$params->get("emailkey", "email key") . $daemailaddress));
				$dalink = $link . "&da=" . $dacode;

				$dnacode = base64_encode($daemailaddress . ":" . md5("directnoattend".$params->get("emailkey", "email key") . $daemailaddress));
				$dnalink = $link . "&dna=" . $dnacode;
				...
				*/
				if (count($damatches) == 2) {
					$message = preg_replace($daregex, "<a href='$dalink'>" . $damatches [1] . "</a>", $message);
					//JFactory::getApplication()->enqueueMessage($daemailaddress." : ".$dalink);

					/*
stlsprout@copyn.plus.com : http://ubu.j33jq.com/index.php?option=com_jevents&task=icalrepeat.detail&evid=5270&Itemid=109&year=2015&month=11&day=23&title=single-click-attendance&uid=9295a7c8c1f7686d6aeed1b941887c7a&da=c3Rsc3Byb3V0QGNvcHluLnBsdXMuY29tOjBiNjM2MTllZjk5ZTRjMDE4OGEwYmQ4NTZhM2Q5MWVk

email@copyn.plus.com : http://ubu.j33jq.com/index.php?option=com_jevents&task=icalrepeat.detail&evid=5270&Itemid=109&year=2015&month=11&day=23&title=single-click-attendance&uid=9295a7c8c1f7686d6aeed1b941887c7a&da=ZW1haWxAY29weW4ucGx1cy5jb206YjM3MjNjOTMzYzcwMDZmYTFkMjUyNzVjNjc5YmZjY2I=&em2=ZW1haWxAY29weW4ucGx1cy5jb206ODY0NDM3OTc5YWI0M2YwMmJmMmFhYzFmNzRhNjIyMTY=
					 */
				}
				if (count($dnamatches) == 2) {
					$message = preg_replace($dnaregex, "<a href='$dnalink'>" . $dnamatches [1] . "</a>", $message);
				}
			...
			function sendMail($from, $fromname, $recipient, $subject, $body, $mode = 0, $cc = null, $bcc = null, $attachment = null, $replyto = null, $replytoname = null)
	{
		...
		if ($this->params->get("invite_icals", 0) == 1 && strpos($body, "BEGIN:VCALENDAR") !== false)
		{
			...
			$body = $body;
        		...
        		$this->prepareMail($mail, $from, $fromname, $recipient, $subject, $body, $mode, $cc, $bcc, $attachment, $replyto, $replytoname);
	...
	/**
	 * Function to prepare an email for sending
	 *
	 * @param   JMail    $mail         Mailer instance
	 ...
	 * @since   11.1
	 */
	public function prepareMail($mail, $from, $fromName, $recipient, $subject, $body, $mode = false, $cc = null, $bcc = null, $attachment = null,
		...
		if (is_array($replyTo))
		{
			...
			{
				$mail->addReplyTo($replyTo[$i], $replyToName[$i]);
			}
		}
		...
		$autoReplyTo = (empty($mail->ReplyTo)) ? true : false;
		$mail->setSender(array($from, $fromName, $autoReplyTo));            
        }
	...
	private function recordRsvpActivity($cmd, &$row)
	{
		...
		CuserPoints::assignPoint('rsvppro.invite');		
		...
		$title = JText::sprintf("JOMSOCIAL_ACTIVITY_INVITED", $link, $row->title());
