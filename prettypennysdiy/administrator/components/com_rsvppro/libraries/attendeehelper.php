...
class RsvpAttendeeHelper
{
	...
	public function adminApproveAttendance($rsvpdata, $row)
	{
		...
		if ($user->id == $row->created_by() || JEVHelper::canPublishEvent($row))
		{
			...
			if ($jevattend_id > 0)
			{
				...
				$sql = "UPDATE #__jev_attendees SET attendstate=1, waiting=0, modified=".$db->Quote($now->toMySql())." WHERE id=" . $attendee->id;
	...
	public function notifyCreator($rsvpdata,& $row, $name, $username, $attendee = null, $cancellation = false, $onWaitingList = false)
	{
		...
		if ($creator)
		{
			...
			$mail_send = $this->sendMail($creator->email, $creator->name, $creator->email, $subject, $message, 1, null, $bcccreator, isset($rsvpdata->pdftickets)?$rsvpdata->pdftickets : null, null, null,true );

		}

	}

	public function notifyUser($rsvpdata, & $row, $user, $name, $username, $attendee = null, $messagetype = 'ack', $onWaitingList = false, $transaction = false, $subject = "", $message = "", $speciallink=false, $replyto=null)
	{
		if (!isset($this->event)) {
			$this->event = $row;
		}
		...
		if ($creator)
		{
			...
			if ($user->id > 0)
			{
				 ...
				 *
				 */
				$success = $this->sendMail($creator->email, $creator->name, $user->email, $subject, $message, 1, null, $notifybcc, isset($rsvpdata->pdftickets)?$rsvpdata->pdftickets : null, $replyto);
				$this->log("Sent message to ".$user->email. " from ".$creator->email. " success = ".($success?"true":"false") );
			}
			else
			{
				$success = $this->sendMail($creator->email, $creator->name, $name, $subject, $message, 1, null, $notifybcc, isset($rsvpdata->pdftickets)?$rsvpdata->pdftickets : null, $replyto);
	...
	public function parseMessage($message, & $rsvpdata,& $row, $name, $username, $attendee = null, $requirelogin = false, $onWaitingList = false, $transaction = false,$speciallink=false, $issubject=false)
	{
		...
		if ($cparams->get("remindplugins", 0)) {
			...
			$dispatcher	= JEventDispatcher::getInstance();
			// allow the message to be customised
			$row->rsvpmessage = $message;
						...
						if ($ticket && stripos($message, "{PDFTICKETS}")!==false){
							...
							if ($pdftickets){
								...
								if (file_put_contents($tmpdir."/tickets".$attendee->id.".pdf", $pdftickets)){
									...
									JFactory::getApplication()->enqueueMessage(JText::_("RSVP_PRO_UNABLE_TO_SAVE_PDF_TICKETS"),"warning");
					...
					if (count($paramsarray) > 0)
					{
						...
						if ($attendee->guestcount > 0)
						{
							...
							foreach ($paramsarray as $param)
							{
								...
								$class= isset($param['classname']) ? "class='".$param['classname']."'" : "";
								$html .= "<tr $class> ";
								$label = stripslashes(RsvpHelper::translate($param['label']));
								// Allow HTML in labels so convert from special characters
								if (strpos($label, "&lt;")!==false){
									$label = htmlspecialchars_decode($label);
								}
								$html .='<td class="rsvpoptionlabel">' . $label . ' : </td>';
		...
		$attendstate = array(JText::_('RSVP_NOT_ATTENDING'), JText::_('RSVP_ATTENDING'), JText::_('RSVP_MAYBE_ATTENDING'), JText::_('RSVP_PENDING_APPROVAL'), JText::_('RSVP_OUTSTANDING_BALANCE'),JText::_("JEV_WAITING_STATE"));
		...
		if ($transaction)
		{
			...
			if (isset($rsvpdata->template) && is_numeric($rsvpdata->template))
			{
				$db = JFactory::getDBO();
				static $tmpls = array();
				if (!isset($tmpls[intval($rsvpdata->template)])){
					$db->setQuery("Select * from #__jev_rsvp_templates where id=" . intval($rsvpdata->template));
					$tmpls[intval($rsvpdata->template)] = $templateParams = $db->loadObject();
				}
				else {
					$templateParams = $tmpls[intval($rsvpdata->template)]; 
				}
				if ($templateParams)
				{
		...
		// remove tmpl=component from messages!!
		$message = str_replace(array('?tmpl=component&', '&tmpl=component','?tmpl=component'), array('?', '', ''), $message);
	...
	public function countAttendees($atdid = 0, $forcecount = false)
	{
		...
		// Move the deletion of attendee count down just before the insert
		//$db->setQuery("DELETE FROM #__jev_attendeecount $where2");
		//$db->execute();
		$db->setQuery("SELECT at_id, rp_id FROM #__jev_attendeecount $where2 ORDER BY at_id asc, rp_id asc");
		$tempdata = $db->loadObjectList();
		$iddata = array();
		foreach ($tempdata as $pair){
			$iddata[$pair->at_id."_".$pair->rp_id] = $pair;
		}
		...
		if (is_array($rows))
		{
			foreach ($rows as $row)
			{
				...
				if ($rsvpdata->template != "")
				{
					...
					if ($templateParams)
					{
						$templateParams = json_decode($templateParams->params);
                                ...
                                // Do we need to cancel unpaid attendees?
                                if ($templateParams && isset($templateParams->autocancelattendees) && intval($templateParams->autocancelattendees)>0) {
                                    $now = JevDate::getDate("-".intval($templateParams->autocancelattendees)." minutes");
                                    $db->setQuery("SELECT * FROM #__jev_attendees WHERE modified<".$db->quote($now->toMySql()). " AND attendstate=4 AND at_id =" . intval($row->at_id) . " AND rp_id=" . intval($row->rp_id));
                                    $attendeesToCancel = $db->loadObjectList();
                                  //  echo $db->getQuery()."<Br/>";
                                    
                                    foreach ($attendeesToCancel as $attendee) {
                                        if ($this->params->get("notifyadmincancelled", 0))
                                        {                                        
                                                $attendeeuser = JEVHelper::getUser($attendee->user_id);
                                                if ($attendeeuser->id == 0 && $this->params->get("attendemails", 0))
                                                ...
                                                $attendee->attendstate = 0;
                                                $this->notifyUser($rsvpdata, $repeat, $attendeeuser, $name, $username, $attendee, 'admincancelled', false);
				...
				$currrentURI = JUri::getInstance();
				$currrentURL = $currrentURI->toString();
				JLog::add("Updated count at_id = $row->at_id, rp_id=$row->rp_id, atdcount=$row->atcnt, gucount = $guestcnt, url = $currrentURL", JLog::INFO, 'rsvppro');
				 */
				$db->setQuery("REPLACE INTO #__jev_attendeecount (at_id, rp_id, atdcount, gucount) VALUES ($row->at_id,$row->rp_id,$row->atcnt, $guestcnt)");
				...
				if (isset($iddata[$row->at_id."_".$row->rp_id])){
					unset($iddata[$row->at_id."_".$row->rp_id]);
				}
			}
		}

		// Delete any unprocessed counts that are out of date
		if (count($iddata)){
			foreach($iddata as $idpair){
				$db->setQuery("DELETE FROM #__jev_attendeecount WHERE at_id = $idpair->at_id AND rp_id = $idpair->rp_id");
				$db->execute();
			}
		}
	...
	public function updateWaitingList($rsvpdata = null, $atdid = 0)
	{
		...
		else if ($atdid > 0)
		{
			...
			foreach ($rows as $row)
			{
				...
				// TODO FIX THIS
				$now = JevDate::getDate("+0 seconds");
				$db->setQuery("UPDATE #__jev_attendees SET waiting=0, modified=".$db->Quote($now->toMySql())." where id=" . $row->atdee_id);
				if ($db->execute())
				{
		...
		if ($rsvpdata->template != "")
		{
			...
			else
			{
				...
				if ($templateParams)
				{
					...
					// Do we have a translation for each of the messages
	...
	public function sendMail($from, $fromname, $recipient, $subject, $body, $mode = 0, $cc = null, $bcc, $attachment = null, $replyto = null, $replytoname = null, $toCreator=false)
	{
		...
		//Check there is a recipient and if not set one if BCC exists return false.
		if(($recipient == '' || !$recipient) && is_array($bcc) && count($bcc) > 0 ) {
			//Get out of here
			$recipient = $bcc[0];
			unset($bcc[0]);
		}

		//Run a check through the array to make sure the recipient isn't in here too
		if (is_array($bcc) && ($key = array_search($recipient, $bcc)) !== FALSE) {
			unset($bcc[$key]);
		}
		//Same as above 1 but just makes sure a recipient is set.
