...
CREATE TABLE IF NOT EXISTS #__jev_invitelist (
	user_id int(11) NOT NULL default 0,
	listname varchar(150) NOT NULL default '',
	id int(11) NOT NULL auto_increment,
...
SQL;
		...
		$charset = ($db->hasUTF()) ? 'DEFAULT CHARACTER SET `utf8`' : '';
		$sql = <<<SQL
CREATE TABLE IF NOT EXISTS #__jev_attendance_translations(
	id int(11) NOT NULL auto_increment,
	atd_id int(11) NOT NULL default 0,
	message text NOT NULL default '',
	subject varchar(255) NOT NULL default '',
...
SQL;
	...
	private function updateTables()
	{
		...
		if (!array_key_exists("classname", $cols))
		{
			$db->setQuery("ALTER TABLE #__jev_rsvp_fields ADD COLUMN classname varchar(255) NOT NULL default '' ");
			@$db->execute();
		}                
		...
		if (!array_key_exists("placeholder", $cols))
		{
			$db->setQuery("ALTER TABLE #__jev_rsvp_fields ADD COLUMN placeholder varchar(255) NOT NULL default '' ");
			@$db->execute();
		}                

		$sql = "SHOW COLUMNS FROM `#__jev_rsvp_fields_archive`";
		$db->setQuery($sql);
		$cols = @$db->loadObjectList("Field");

                if (!array_key_exists("placeholder", $cols))
		{
			$db->setQuery("ALTER TABLE #__jev_rsvp_fields_archive ADD COLUMN placeholder varchar(255) NOT NULL default '' ");
			@$db->execute();
		}                

		$sql = "SHOW COLUMNS FROM `#__jev_rsvp_fields_translation`";
		$db->setQuery($sql);
		$cols = @$db->loadObjectList("Field");

                if (!array_key_exists("placeholder", $cols))
		{
			$db->setQuery("ALTER TABLE #__jev_rsvp_fields_translation ADD COLUMN placeholder varchar(255) NOT NULL default '' ");
			@$db->execute();
		}                
