...
class AdminTemplatesViewTemplates extends RSVPAbstractView
{
	...
	function edit($tpl = null){
		...
		if (JFile::exists(JPATH_SITE . "/components/com_jevents/assets/css/jevcustom.css"))
		{
			...
			JEVHelper::stylesheet('jevcustom.css', 'components/' . JEV_COM_COMPONENT . '/assets/css/');
                ...
                $copyTranslations = 0;
                if (isset($item->id) && $item->id>0 && (JRequest::getInt("customise",0) || JRequest::getString("task")=="templates.editcopy" )) {
                    $db = JFactory::getDbo();
                    $db->setQuery("SELECT count(translation_id) FROM #__jev_rsvp_fields_translation where template_id=".$item->id);
                    $copyTranslations = intval($db->loadResult())>0;
                }
                $this->assignRef('copyTranslations',$copyTranslations);
		parent::displaytemplate($tpl);
	...
	function translate($tpl = null)
	{
		...
		if ($this->editor->get("_name") == "codemirror")
		{
			...
			if (!$this->editor) {
				JFactory::getApplication()->enqueueMessage(JText::_("JEV_EDITOR_NONE_NOT_PUBLISHED", "WARNING"));
			}
		}
