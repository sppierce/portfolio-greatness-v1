...
?>
		...
		if (form.title.value == "") {
				...
				<fieldset class="jevtemplatefields">
					...
					<div class="largeDetail">
						<div class="largeDetailContent">	
                                                    <div class="form-horizontal span12">
                                                        <?php
                                                        // if this is a customisation do we want to copy any field translations
                                                        if ($this->copyTranslations){
                                                            ?>
                                                            <div class="span12" id="jevtemplate_fields_translations">
                                                                <h3><?php echo JText::_('RSVP_FIELD_TRANSLATIONS'); ?></h3>
                                                                    <div class="control-group">
                                                                            <label class="control-label"><?php echo JText::_('RSVP_COPY_FIELD_TRANSLATIONS'); ?></label>
                                                                            <div class="controls">
                                                                                    <div class="radio btn-group">
                                                                                        <label  for="copytranslations1" class="btn radio"><?php echo JText::_("JEV_YES"); ?><input class="inputbox" type="radio" name="copytranslations"  id="copytranslations1"  value="1"  /></label>
                                                                                        <label  for="copytranslations0" class="btn radio"><?php echo JText::_("JEV_NO"); ?><input class="inputbox" type="radio" name="copytranslations"  id="copytranslations0"  value="0"  checked='checked' /></label>
                                                                                    </div>
                                                                            </div>
                                                                    </div>                                                                
                                                            </div>
							...
							<input id="closeFieldButton" type="button" value="<?php echo JText::_("RSVP_CLOSE_FIELD"); ?>" style='display:none' class="closeFieldButton"/>
                                                        ...
                                                        <div class="span6">
                                                            <h3><?php echo JText::_('RSVP_select_existing_field'); ?></h3>
                                                            <?php
			JLoader::register('JevTypeahead',JPATH_LIBRARIES."/jevents/jevtypeahead/jevtypeahead.php");
			$datapath = JRoute::_("index.php?option=com_rsvppro&ttoption=com_rsvppro&typeaheadtask=gwejson&file=findtemplatefield", false);
			JevTypeahead::typeahead('#sessiontemplate',array('remote'=>  $datapath,
					'data_value'=>'title',
					'data_id' => 'id',
					'field_selector'=>'#existingfield',
					'minLength'=>3,
					'limit'=>10,
					'scrollable'=>1,
					//'emptyCallback' => 'noMatchingLocations()'
                                        ));
			$typeaheadLoad = "var sessionTemplateSelected = false;
                                jQuery(document).ready(function() {
                                    jQuery('#sessiontemplate').on
                                            (
                                            'typeahead:change',                                        
                                            function()
                                            {  
                                                sessionTemplateSelected = false;
                                            }
                                    );
                                    jQuery('#sessiontemplate').on
                                            (
                                            ...
                                            function(ev, data)
                                            {   
                                                sessionTemplateSelected = true;
                                                
                                                if (data.html != undefined){                                                    
                                                    var id = rsvpforms.uniqId('field');
                                                    var newDiv = jQuery(data.html.replace('field'+data.id, id));
                                                    ...
                                                    // make sure this field is highlighted and others not
                                                    rsvpforms.highlightField(id);
                                                    ...
                                                    setupRSVPTemplateBootstrap();
                                                     jQuery('#sessiontemplate').eq(0).val('').trigger('input');
                                                }
                                    ...
                                    jQuery('#sessiontemplate').on
                                            ...
                                            function()
                                            {  
                                                var title = jQuery('#sessiontemplate').val();
                                                var id = jQuery('#existingfield').val();
                                                if (title.indexOf('#')>0 || title.length==0){
                                                }
                                                else if (sessionTemplateSelected) {
                                                    jQuery('#sessiontemplate').eq(0).val(title+'#').trigger('input');
                                                }
                                            }
                                    )
                                });";
			JFactory::getDocument()->addScriptDeclaration($typeaheadLoad);
                                                            ...
                                                            ?>
                                                            <input type="hidden" name="existingfield" id="existingfield" value="" />                   
                                                            <div id="scrollable-dropdown-menu">
                                                                <input name="template_notused"  id="sessiontemplate" class="jevtypeahead" value=""  type="text" autocomplete="off" placeholder="<?php echo JText::_("RSVP_TEMPLATE_COLON_FIELD"); ?>">
                                                            </div>
                                                        </div>                                                        
                                                    </div>
							<div id="rsvpfields">
