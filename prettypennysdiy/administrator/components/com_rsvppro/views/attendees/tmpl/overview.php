...
if (!empty($this->sidebar))
{
	...
	<table cellpadding="4" cellspacing="0" border="0" width="100%" class="adminlist  table table-striped" id="attendeelist">
		...
		$didattend .= " <br/>" . $attendee->guestattend;
					}
					echo $didattend;
					?>
				    </td>
				    <?php
				    if (!isset($attendee->atdee_id))
				    {
					    $attendee->notes = "";
			    }
			    ?>
				    <td  <?php echo $rowspan; ?>>
					<textarea rows="2" cols="10" name="notes[<?php echo $attendee->atdee_id; ?>]" id="attendeenotes_<?php echo $attendee->atdee_id; ?>" onkeyup="jQuery('#attendeenotes_<?php echo $attendee->atdee_id; ?>').parent().find('div').css('display', 'block');" ><?php echo trim($attendee->notes); ?></textarea>
					<div >
					    <a class="btn btn-micro active" href="javascript:void(0);" onclick="return listItemTask('cb<?php echo $i; ?>', 'attendees.updatenotes')" >
						<span class="icon-save"></span>
						<?php echo JText::_("RSVP_UPDATE_NOTES"); ?>
					    </a>
					</div>
				    </td>
				    <?php
