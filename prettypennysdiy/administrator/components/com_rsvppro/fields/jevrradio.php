...
class JFormFieldJevrradio extends JevrFieldRadio
{
	...
	public static function loadScript($field = false)
	{
		...
		ob_start();
		?>
		<div class='rsvpfieldinput  rsvpradiofield'>
	...
	function getInput()
	{
		...
		if ($this->attribute("peruser") == 1 || $this->attribute("peruser") == 2)
		{
			...
			foreach ($value as $val)
			{
				...
				$thisclass .= " id = 'rsvp_" . $fieldname . "_span_$i' ";
                                $inputattribs = " onchange='JevrConditionFieldState.changeState(jQuery(this).parent(), \"" . $this->fieldname . "\");' ";
				$html .= JHtml::_('jevrList.radiolist', $options, $elementname, $thisclass, 'value', 'text', $val, $id . "_" . $i, $inputattribs);
			...
			$thisclass .= " id = 'rsvp_" . $fieldname . "_span_xxxyyyzzz' ";
                        $inputattribs = $thisclass . " onchange='JevrConditionFieldState.changeState(jQuery(this).parent(),\"" . $this->fieldname . "\");' ";
			$html .= JHtml::_('jevrList.radiolist', $options, "paramtmpl_" . $elementname, $thisclass, 'value', 'text', $val, $id . "_xxxyyyzzz", $inputattribs);
		...
		else
		{
			...
			$thisclass .= " id = 'rsvp_" . $fieldname . "_span_' ";
                        $inputattribs = " onchange='JevrConditionFieldState.changeState(jQuery(this).parent(),\"" . $this->fieldname . "\");' ";
			$html = JHtml::_('jevrList.radiolist', $options,  $name, $thisclass, 'value', 'text', $value, $id . "_", $inputattribs);
			$script = "JevrConditionFieldState.append( { '" . $this->fieldname . "' : {'name':'" . $this->fieldname . "', 'value':'" . $value . "',  'peruser' :" . $peruser . "}}); ";
			$doc = JFactory::getDocument();
			$doc->addScriptDeclaration($script);
		}
