...
class JFormFieldJevrcheckbox extends JevrFieldCheckbox
{
	...
	public static function loadScript($field = false)
	{
		...
		?>
			...
			<?php
			RsvpHelper::requiredAnyAll($id, $field);
	...
	function fetchRequiredScript()
	{
		...
		if ($this->attribute("peruser") == 1 || $this->attribute("peruser") == 2)
		{
			...
			foreach ($value as $val)
			{
				...
				$script .= "jevrsvpRequiredFields.fields.push({'requiredtype':'".$this->attribute("required")."', 'name':'" . $elementname . "', 'type':'checkbox', 'id':'" . $elementid . "',  'default' :'" . $this->attribute("default") . "' ,'reqmsg':'" . trim(JText::_($this->attribute('requiredmessage'), true)) . "'}); ";
				break;
				$i++;
			}
		}
