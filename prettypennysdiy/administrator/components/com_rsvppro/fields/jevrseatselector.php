...
$lang->load("seats",JPATH_ADMINISTRATOR ."/components/com_rsvppro/fields/SeatSelector");

include_once(JPATH_ADMINISTRATOR ."/components/com_rsvppro/fields/jevrlist.php");

class JFormFieldJevrseatselector extends JFormFieldJevrlist
{
	 ...
	 * @var		string
	 */
	var $_name = 'jevrseatselector';
	const name = 'jevrseatselector';
	...
	public static function loadScript($field=false)
	{
		...
		<div class='rsvpfieldinput'>
			...
			<div class="rsvpinputs" style="font-weight:bold;"><?php echo JText::_("RSVP_TEMPLATE_TYPE_jevrseatselector"); ?><?php RsvpHelper::fieldId($id); ?></div>
			...
			<div class="rsvpinputs" style="height:200px;overflow-y:auto ">
				...
				if ($field && $field->options != "")
				{
					...
					$optionvalues->label[] = "Please Select a Seat";
					$optionvalues->capacity[] = 0;
					$optionvalues->price[] = 0;
					$optionvalues->value[] = 0;
					$count=0;
					
					$rows = array("B","C","D","E","F","G","H","J");
					$count ++;
					foreach ($rows as $row){
						for ($seat=1;$seat<26;$seat++){
							if ($seat==1 && $row=="J") continue;
							if ($seat>22 && ($row=="A" || $row=="B" || $row=="C") ) continue;
							if ($seat>24 && ($row=="D" || $row=="E" || $row=="F" || $row=="G") ) continue;
							if ($seat>23 && ($row=="H") ) continue;
							// disabled seats
							if (in_array($seat, array(1,2,19,20)) && ($row=="D") ) continue;
							$optionvalues->label[] = $row.$seat;
							$optionvalues->capacity[] = 1;
							$optionvalues->price[] = 0;
							$optionvalues->value[] = $count;
							$count ++;
						}
					}
				}
	...
	function getInput()
	{
		...
		$html = "<a href='".JURI::root()."administrator/components/com_rsvppro/fields/SeatSelector/seats.php?field=$id&rpid=".$row->rp_id()."&at_id=" . $rsvpdata->id.$atdee."' title='Select Seat' class='jevmodal btn' rel='{handler: \"iframe\", size: {x: 750, y: 470}}'>Select a seat</a>" . $html;
		return $html;
