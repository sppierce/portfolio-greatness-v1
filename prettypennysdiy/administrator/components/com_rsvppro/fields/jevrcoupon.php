...
class JFormFieldJevrcoupon extends JevrFieldList
{
	...
	public static function loadScript($field=false)
	{
		...
		<div class='rsvpfieldinput'>
			...
			<div class="rsvpinputs">
				...
				// add 10 blank options at the end
				for ($op = 0; $op < 10; $op++)
				{
					$option = new stdClass();
	...
	function getInput()
	{
		...
		$couponURL = JRoute::_("index.php?option=com_jevents&typeaheadtask=gwejson&file=checkcoupon&path=plugin&folder=jevents&plugin=jevrsvppro/rsvppro");
		$action = 'checkCoupon(event,jQuery(\'#' . $id.'\'),\''.$couponURL.'\', \''.$client.'\', \''.$fieldid.'\', \''.$this->event->rp_id().'\', \''.$this->rsvpdata->id.'\', \''.$attdendeeid.'\')';
		if ($this->isEarlyBird){
...
/*
		...
		$elementname = $this->name;
                ...
                JFactory::getDocument()->addScriptDeclaration("jevrsvpRequiredFields.fields.push({'name':'" . $elementname. "', 'type':'radio', 'id':'" . $elementid . "', 'default' :'" . $this->attribute("default") . "' ,'reqmsg':'" . trim(JText::_($this->attribute('requiredmessage'), true)) . "'}); ");
*/
		return $html;
	...
	function fetchBalanceScript( $value)
	{
		...
		{
			$appliesperuser = $this->attribute("appliesperuser", 0);
			$pricefunction = " function(name){if (JevrConditionalFields && !JevrConditionalFields.isVisible(name, 0)) {return 0;}  if ($appliesperuser==1) { return jQuery('#guestcount').val() * ".$id."discount; } \nelse if ($appliesperuser==2)  { return (jQuery('#guestcount').val()-1) * ".$id."discount; } \n else return ".$id."discount;}";
	...
	public static function calendar($value, $name, $id, $format = '%Y-%m-%d', $attribs = null)
	{
		...
		if ($done === null)
		{
			...
			//TODO remove the below in Joomla! 3.7.5
			...
			$script = <<<SCRIPT
 function clonecalendar(elem){
	var imgid = elem.id;
	var fieldid = elem.id.replace("_img","");
...
SCRIPT;
		...
		$imgattribs = array('class' => 'calendar ' . (isset($attribs["class"])?$attribs["class"]:""), 'id' => $id . '_img', 'onload' => 'jQuery(this).delay(1100).queue(function(){clonecalendar(this);})');                
