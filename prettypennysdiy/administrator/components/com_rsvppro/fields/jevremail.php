...
<div class='rsvpfieldinput'>
	...
	<?php
	RsvpHelper::placeholder($id,  $field);
...
<div class="rsvpclear"></div>
	...
	function getInput()
	{
		...
		$class = ( $this->attribute('class') ? 'class="'.$this->attribute('class').' xxx"' : 'class="text_area xxx"' );
                $placeholder = ( $this->attribute('placeholder') ? 'placeholder="'.htmlspecialchars($this->attribute('placeholder')).'"' : 'placeholder=""' );
		...
		if ($this->attribute("peruser")==1 || $this->attribute("peruser")==2){
			...
			foreach ($value as $val){
				...
				$html .= '<input type="text" '.$placeholder.' name="'.$elementname.'" id="'.$id."_".$i.'" value="'.$val.'" '.$thisclass.' '.$size.' '.$maxlength.'  '.$change.' />';
				$i++;
			}
			$val = $this->attribute("default");
			$thisclass = str_replace(" xxx"," paramtmpl rsvp_" . $fieldname,$class);
			$html .= '<input type="text" '.$placeholder.' name="paramtmpl_'.$elementname.'" id="'.$id.'_xxx" value="'.$val.'" '.$thisclass.' '.$size.' '.$maxlength.'  '.$change.' />';
		}
		else {
			// this is the attendee email address
			...
			$thisclass = str_replace(" xxx", " rsvpparam rsvpparam0 rsvp_" . $fieldname, $class);
			$html = '<input type="text" '.$placeholder.' name="'.$name.'" id="'.$id.'" value="'.$value.'" '.$thisclass.' '.$size.' '.$maxlength.'  '.$change.' />';
		}
		return $html;
	}
