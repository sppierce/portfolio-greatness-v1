...
<?php
		...
		foreach ($this->element->children() as $option)
		{
			...
			if ($text=="Manual (Admin Only)" && !JFactory::getApplication()->isAdmin()){
				continue;
                        ...
                        $paymentPluginClass = "plgRsvppro" . ucfirst($val);
                        $paymentPluginPath = JPATH_SITE . "/plugins/rsvppro/$val/" ;
                        JLoader::register($paymentPluginClass, $paymentPluginPath . $val . ".php");
                        if (is_callable(Array($paymentPluginClass,"setupProductInAdvance"))) {
                            $paymentPlugin = JPluginHelper::getPlugin("rsvppro",$val);
                            $dispatcher = JEventDispatcher::getInstance();
                            $paymentPlugin = new $paymentPluginClass($dispatcher, (array) ($paymentPlugin));
                            $paymentPlugin->setupProductInAdvance($rsvpdata, $row->rp_id(), $row->ev_id(), $row);
                        }
	 ...
	 */
	/*
	public function render_NOT_USED(&$xmlElement, $value, $control_name = 'params')
	{
		// Deprecation warning.
