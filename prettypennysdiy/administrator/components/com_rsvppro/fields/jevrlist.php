...
class JFormFieldJevrlist extends JevrFieldList
{
	...
	protected static $overridenPrice;
			...
			<div class="rsvpinputs">
				...
				if (isset($optionvalues))
				{
					...
					foreach ($optionvalues->label as $lab)
					{
						...
						$option->label = htmlspecialchars($lab,ENT_COMPAT | ENT_HTML401, "UTF-8", false);
				...
				<table id="options<?php echo $id; ?>" class="sortablelistoptions">
					<tr class="sortablelabels">
	...
	function getInput()
	{
		...
		$includeunpaid = "";
		if ($templateParams && isset($templateParams->unpaidcapacity) && $templateParams->unpaidcapacity)
		{
			$includeunpaid = " OR a.attendstate=4";
		}

		$sql = "SELECT params FROM #__jev_attendees as a WHERE a.at_id=" . $rsvpdata->id . " AND (a.attendstate=1 $includeunpaid ) AND waiting=0";
		...
		$setcapacity = intval($this->attribute("capacity"))==0;
		...
		foreach ($this->element->children() as $option)
		{
			...
			// If the field capacity attribute is blank/set to zero but we have line capacities then do a workaround here
			if ($setcapacity) {
				$this->addAttribute('capacity',$this->attribute("capacity") + intval( $option['capacity'])*intval($val));
			}

			// field specific capacity too high - this is handled later by disabling this option!
			...
			$newoptions[$htmloption->value] = $htmloption;
		}
		if (($rsvpdata->capacity || $this->attribute("capacity")>0) && (count($newoptions)==0 || (count($newoptions)==1 && isset($newoptions[0]) && $newoptions[0]->value==0))){
		...
		foreach ($optionlabels as $p => $text)
		{
			if ($text!="")
			{
				...
				$hasSpaceLeft = true;
                                ...
                                // Now adjust labels to show how many options are left
				$spacesleft = 0;
				...
				else if ($p >= 0 && $capacities[$p] > 0 && $allcounts[$p] >= $capacities[$p] && $waiting[$p] > 0)
				{
					...
					if ((is_array($value) && $value[0] != $p) || (!is_array($value) && $value != $p))
					{
				...
				if ($p >= 0 
                                        && $capacities[$p] > 0 
                                        && $allcounts[$p] >= $capacities[$p] 
                                        && ((is_array($value) && $value[0] != $p) || (!is_array($value) && $value != $p))
                                        && $waiting[$p] > 0)
				{
				...
				$options[] = $htmloption;
                                if (isset($newoptions[$htmloption->value])){
                                    $newoptions[$htmloption->value] = $htmloption;
                                }
                                else {
                                    $newoptions[$htmloption->value] = $htmloption;
		...
		if ($this->attribute("peruser") == 1 || $this->attribute("peruser") == 2)
		{
			...
			foreach ($value as $val)
			{
				...
				foreach ($newoptions as $htmloption){
	...
	private function setPrices() {
		...
		// is there an overridden price per attendee from the event edit page
		$overrideUnitPrice = $this->overridePrice();
		...
		if (!isset($this->hasPricesData[$name]))
		{
			...
			foreach ($this->element->children() as $option)
			{
				...
				if (!is_null($price))
				{
					// should we replace the option price with a set price based on how many tickets
					// Special code for hcharles
					//$prices[$val] = ($overrideUnitPrice) ? ($val-1) * $overrideUnitPrice : $price;
					$prices[$val] = $price;
			...
			foreach ($this->element->children() as $option)
			{
				...
				$price =  (string) $option['price'];
                                if (is_numeric($price) && $price != 0){
                                    $text .= " (".RsvpHelper::phpMoneyFormat($price).")";
                                }
	...
	private function overridePrice()
	{
		// disable for list fields for now
		return;
		// overriding price is used only for fields which value is included in capacity count
		if (!$this->attribute("includeintotalcapacity"))
		{
			return 0;
		}

		// have we overridden the price ??
		if (!isset(self::$overridenPrice)) {
			self::$overridenPrice = false;
		}

		// is there an overridden price alwasy apply it to this field
		if ((!self::$overridenPrice  || self::$overridenPrice  == $this->fieldname) && isset($this->rsvpdata) && isset($this->rsvpdata->overrideprice) && trim($this->rsvpdata->overrideprice) !== "")
		{
			//$this->value = floatval($this->rsvpdata->overrideprice);
			self::$overridenPrice  = $this->fieldname;
			return (floatval($this->rsvpdata->overrideprice));
		}
		else
		{
			return 0;
...
class JHTMLJevrList
{
	 ...
	 * @returns string HTML for the select list
	 */
	public static function radiolist($arr, $name, $attribs = null, $key = 'value', $text = 'text', $selected = null, $idtag = false, $inputattribs="")
	{
