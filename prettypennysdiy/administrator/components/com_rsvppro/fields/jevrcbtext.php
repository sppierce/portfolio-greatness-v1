		...
		if ($showinform)
		{
			...
			if ($this->attribute("peruser")==1 || $this->attribute("peruser")==2){
				...
				foreach ($value as $val){
					...
					$html .= '<input type="text" name="'.$elementname.'" id="'.$id."_".$i.'" value="'.$val.'" '.$thisclass.' '.$size.' '.$maxlength.'  />';
					$i++;
				}
				if ($this->attribute("default")!=""){
					$default = $this->attribute("default");
				...
				$html .= '<input type="text" name="paramtmpl_'.$elementname.'" id="'.$id.'_xxx" value="'.$val.'" '.$thisclass.' '.$size.' '.$maxlength.'  />';
			...
			$html = '<input type="text" name="'.$elementname.'" id="'.$id.'" value="'.$value.'" '.$thisclass.' '.$size.' '.$maxlength.' />';

		}
		return $html;
	}

	function render_NOT_USED(&$xmlElement, $value, $control_name = 'params')
	{
