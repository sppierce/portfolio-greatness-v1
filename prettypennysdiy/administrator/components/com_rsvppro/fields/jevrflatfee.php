...
class JFormFieldJevrflatfee extends JevrField
{
	...
	public static function loadScript($field=false)
	{
		...
		ob_start();
                ?>
                    ...
                    jevrflatfee.setvalue('<?php echo $id; ?>');
                });
                </script>
		...
		<div class='rsvpfieldpreview'  id='<?php echo $id; ?>preview'>
			...
			<div id="pdv<?php echo $id; ?>">
				<?php echo  RsvpHelper::phpNewMoneyFormat($field ? $field->defaultvalue : ""); ?>
			</div>
		</div>
		<div class="rsvpclear"></div>
		<?php
		...
		if ($conditionnode)
		{
			...
			if (isset($attendeefields->$cf))
			{
				...
				else
				{
					if ($this->attribute("peruser") == 1)
					{
						if (is_object($cfieldvalue)) {
							$cfieldvalue = get_object_vars($cfieldvalue);
						}
	...
	private function overridePrice()
	{
		...
		// Make sure we always override this particular field
		if ((!self::$overridenPrice  || self::$overridenPrice  == $this->fieldname) && isset($this->rsvpdata) && isset($this->rsvpdata->overrideprice) && trim($this->rsvpdata->overrideprice)!==""){
			...
			$this->addAttribute("amount",  $this->value);
			$this->addAttribute("default",  $this->value);
