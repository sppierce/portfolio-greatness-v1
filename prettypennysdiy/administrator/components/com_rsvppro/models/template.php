...
class TemplatesModelTemplate extends JModelAdmin
{
	...
	function _loadData()
	{
		...
		{
			$query = 'SELECT w.* FROM #__jev_rsvp_templates AS w WHERE w.id = '. intval( $this->_id);
			$this->_db->setQuery($query);
			...
			if ($this->_data){
				...
				$query = 'SELECT w.* FROM #__jev_rsvp_fields AS w WHERE w.template_id = '.intval( $this->_dataobject->id). " ORDER BY ordering ASC";
				$this->_db->setQuery($query);
								...
								foreach (get_object_vars($translatedoptions) as $optkey => $optval){
									if (is_array($optval)) {
										foreach ($optval as $optvalkey => $optvalval){
											...
											if (isset($rawoption[$optvalkey]) && !is_numeric($rawoption[$optvalkey]) &&  $rawoption[$optvalkey] != "" && $optvalval !=""){
