 ...
 * @subpackage Plugin
 */
class plgDroppicsMasonry extends droppicsPluginBase
{
    ...
    public function onShowFrontGallery($options){
        ...
        $stylesheets[] = JURI::base('true').'/plugins/droppics/masonry/style.css';
        ...
        $scripts[] = JURI::base('true').'/plugins/droppics/masonry/jquery.masonry.min.js';
        ...
        $scripts[] = JURI::base('true').'/plugins/droppics/masonry/script.js';
        ...
        if(!empty($this->options['pictures']) || !empty($this->options['categories'])){
            if ($this->options['listchid']){
                ...
                $content = '';
                $content .= '<div id="droppicsgallery'.$this->options['id_gallery'].'" data-id="'.$this->options['id_gallery'].'" class="droppicsgallery droppicsgallerymasonry droppicslightbox" data-useinfinite="'.$useInfinite.'" data-infiniteajax="'.(int)$infiniteAjaxLoad.'" data-infinitefirst="'.(int)$infiniteFirstLoad.'" data-listchid="'.$listchid.'">';
                ...
                foreach ($this->options['pictures'] as $picture){
                    ...
                    $picture->picture_params = json_decode($picture->picture_params);
                    $width = '';

                    $colwidth = (int)droppicsBase::loadValue(@$this->options['params'],'masonry_colwidth',200);
                    if(isset($picture->picture_params->masonry_image_width)){
                        $imageForceWidth = abs((int)$picture->picture_params->masonry_image_width);
                    }else{
                        $imageForceWidth =0;
                    }
                    $imageinfos = getimagesize(COM_MEDIA_BASE.'/'.$this->root_folder().'/'.$picture->id_gallery.'/thumbnails/'
                        .$picture->file);
                                        
                    if($imageForceWidth && $imageForceWidth>$imageinfos[0]){
                        $thumb = '/';
                        $width = 'style="width:'.$imageForceWidth.'px;"';
                    }elseif(!$imageForceWidth && $colwidth>$imageinfos[0]){
                        $thumb = '/';
                    }else{
                        $thumb = '/thumbnails/';
                        if($imageForceWidth){
                    ...
                    $cls = "";
                    $img = '<img class="img" '.$width.' src="'.COM_MEDIA_BASEURL.'/'.$this->root_folder().'/'.$picture->id_gallery.$thumb.$picture->filever.'" alt="'.$picture->alt.'" title="'.$picture->title.'" />';
                    ...
                    if(isset($pparams->is_video) && $pparams->is_video=="1" ) {
                        ...
                        $contentp = '<div class="wimg droppicsvideo" '.$width.'>';
                        $contentp .=    '<a class="'.$cls.'" data-vwidth="'.$vwidth.'" data-vheight="'.$height.'" href="'.$pparams->directLink .'" rel="lightbox['.$picture->id_gallery.']" '.$datatitle.'>'.$img.'<i class="video"></i></a>';
                        $contentp .= '</div>';
                    }
                    elseif($custom_link) {
                        ...
                        $contentp = '<div class="wimg" '.$width.'>';
                        $contentp .=    '<a '.$width.'class="'.$cls.'" href="'.$custom_link.'" '.$datatitle.'>'.$img.'</a>';
                        $contentp .= '</div>';
                    }else {
                        $contentp = '<div class="wimg droppicslightbox" '.$width.'>';
                        $contentp .=    '<a '.$width.' class="'.$cls.'" href="'.COM_MEDIA_BASEURL.'/'.$this->root_folder().'/'.$picture->id_gallery.'/'.$picture->filever.'" rel="lightbox['.$picture->id_gallery.']" '.$datatitle.'>'.$img.'</a>';
                        $contentp .= '</div>';
                    }
            ...
            if((!empty($this->options['categories']) || !empty($this->options['parent'])) && droppicsBase::loadValue($options['params'],'masonry_show_subcategories','1')){
    ...
    protected function addStyleDeclaration($options,$dynamic = false){
            ...
            $style = '';
            $style .= '#droppicsgallery'.$this->options['id_gallery'].' .img {';
            $style .= '     width : '.droppicsBase::loadValue($params,'masonry_colwidth',200).'px;';
            $style .= '     border-radius : '.droppicsBase::loadValue($params,'masonry_radius',0).'px;';
            $style .= '     --moz-border-radius : '.droppicsBase::loadValue($params,'masonry_radius',0).'px;';
            $style .= '     --webkit-border-radius : '.droppicsBase::loadValue($params,'masonry_radius',0).'px;';
            $style .= '}';
            
            $style .= '#droppicsgallery'.$this->options['id_gallery'].' .wimg > a {';
            $style .= '     width : '.droppicsBase::loadValue($params,'masonry_colwidth',200).'px;';
            $style .= '}';
            
            $style .= '#droppicsgallery'.$this->options['id_gallery'].' .droppicspictures .wimg {';
            $style .= '     margin : '.droppicsBase::loadValue($params,'masonry_margin',2).'px;';     
            $style .= '     padding : '.droppicsBase::loadValue($params,'masonry_border',0).'px;';
            if(droppicsBase::loadValue($params,'masonry_border',0)>0){
                $style .= '     background : '.droppicsBase::loadValue($params,'masonry_background','#CCC').';';
            }
            $style .= '     border-radius : '.droppicsBase::loadValue($params,'masonry_radius',0).'px;';
            $style .= '     --moz-border-radius : '.droppicsBase::loadValue($params,'masonry_radius',0).'px;';
            $style .= '     --webkit-border-radius : '.droppicsBase::loadValue($params,'masonry_radius',0).'px;';
            $style .= '}';
            $style .= '#droppicsgallery'.$this->options['id_gallery'].' .wcat span {';
            $style .= '     width : '.droppicsBase::loadValue($params,'masonry_colwidth',200).'px;';
            $style .= '     margin : '.droppicsBase::loadValue($params,'masonry_border',0).'px;';
            $style .= '}';
            $style .= '#droppicsgallery'.$this->options['id_gallery'].'.droppicsgallerymasonry .droppicscats .wimg {';
            ...
            $style .= '}';
            $style .= '.droppicsgallerymasonry .droppicscatslink span {';
