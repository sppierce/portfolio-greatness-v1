...
class JevRsvpDisplayAttendance
{
	...
	public function displayCustomFields(&$row)
	{
		...
		if ($eventid > 0)
		{
			...
			if ($rsvpdata)
			{
                                $sql = "SELECT * FROM #__jev_attendance_translations WHERE atd_id=" . $rsvpdata->id. " AND language=".$db->quote(JFactory::getLanguage()->getTag());
            			$db->setQuery($sql);
                		$translatedRsvpdata = $db->loadObject();
                                if ($translatedRsvpdata) {
                                    foreach (get_object_vars($translatedRsvpdata) as $k => $v){
                                        if ($k != "id" && $k != "atd_id" and $k != "language" && $v != ""){
                                            $rsvpdata->$k = $v;
                                        }
                                    }
                                }
				if ($rsvpdata->allowregistration) {
                                        ...
                                        // We want to be able to respond to specific situation e.g. a google conversion code so catch them here
                                        // REALLY klunky way of doing this !
                                        $messages = JFactory::getApplication()->getMessageQueue();
                                        foreach ($messages as $message) {
                                            if ($message["message"]==JText::_("JEV_ATTENDANCE_CONFIRMED")){
                                                JFactory::getApplication()->input->setVar("rsvp_registered",1);
                                            }
                                        }
				...
				$this->processDirectAttendanceClick($row);
	...
	protected function processDirectAttendanceClick ($row) {
		$dadata = JRequest::getString("da","");
		$dnadata = JRequest::getString("dna","");

		if ($dadata  || $dnadata) {
			$directdata = $dadata ? @base64_decode($dadata) : @base64_decode($dnadata);
			if ($directdata && count(explode(":", $directdata))==2){
				list($daemailaddress, $directcode) = explode(":", $directdata);
				$params = JComponentHelper::getParams("com_rsvppro");
				$checkdacode = base64_encode($daemailaddress . ":" . md5("directattend".$params->get("emailkey", "email key") . $daemailaddress));
				$checkdnacode = base64_encode($daemailaddress . ":" . md5("directnoattend".$params->get("emailkey", "email key") . $daemailaddress));
				if ($checkdacode == $dadata || $checkdnacode == $dnadata) {
					//echo $daemailaddress . ($dadata ? " is attending" : " is not attending");
					...
					if ($rsvpdata) {
						...
						$rp_id = $row->rp_id();
						$hasrepetition = $row->hasrepetition();

						// Find matching invitation
						$db = JFactory::getDbo();
						$sql = "SELECT id , email_address as email , 0 as user_id from #__jev_invitees WHERE email_address = ".$db->Quote(strtolower($daemailaddress)). " AND at_id=" . $rsvpdata->id;
						if (!$rsvpdata->allinvites && $hasrepetition)
						{
							$sql .= " AND rp_id=" . $rp_id;
						}
						$db->setQuery($sql);
						$invitee = $db->loadObject();
						if (!$invitee) {
							// May be a user based invitee
							$sql = "SELECT i.id , u.email , i.user_id from #__users as u "
									. " LEFT JOIN #__jev_invitees as i ON u.id=i.user_id "
									. " WHERE u.email = ".$db->Quote(strtolower($daemailaddress)). " AND i.at_id=" . $rsvpdata->id;
							if (!$rsvpdata->allinvites && $hasrepetition)
							{
								$sql .= " AND rp_id=" . $rp_id;
							}
						...
						if ($invitee) {
							// First step marked as viewed
							$sql = "UPDATE #__jev_invitees set viewedevent=1  WHERE id =" . $invitee->id ;
							$db->setQuery($sql);
							$db->execute();

							// Step 2 check  if we already have this attendee
							if ($invitee->user_id){
								$sql = "SELECT * FROM #__jev_attendees WHERE at_id=" . $rsvpdata->id . " and user_id=" . $invitee->user_id;
							}
							else {
								$sql = "SELECT * FROM #__jev_attendees WHERE at_id=" . $rsvpdata->id . " and LOWER(email_address)=" . $db->Quote($invitee->email);
							}
							...
							if ($testattendee) {
								$sql = "UPDATE #__jev_attendees SET attendstate = ".($dadata ? 1 : 0) ." WHERE id=" . $testattendee->id  ;
							}
							else {
								...
								$sql = "INSERT INTO #__jev_attendees SET attendstate = ".($dadata ? 1 : 0) .", at_id=" . $rsvpdata->id . ", user_id=" . $invitee->user_id .", email_address=" . $db->Quote($invitee->email) .", created=" . $db->Quote($created). ", modified=" . $db->Quote($created);
                                                                if (!$rsvpdata->allrepeats)
                                                                {
                                                                        $sql .= ", rp_id=" . $rp_id;
                                                                }
							...
							$attendeehelper ->countAttendees($rsvpdata->id);

							JFactory::getApplication()->enqueueMessage($dadata ? JText::_('JEV_ATTENDANCE_RECORDED_MAY_NEED_TO_LOGIN') : JText::_('JEV_NOT_ATTENDANCE_RECORDED_MAY_NEED_TO_LOGIN'));
						}
					}
				}
		...
		if (strtolower($row->freq())=="none"){
			...
			if (!isset($repeat->ex_id) || $repeat->ex_id==0){
				...
				return;
                            ...
                            // skip non time changing exceptions
                            if (isset($repeat->_exception_type) && $repeat->_exception_type==1 && $repeat->_exstartrepeat == $repeat->_exoldstartrepeat) {
                            // set dtstart to match the first repeat
				$row->dtstart($repeat->getUnixStartTime());
				$row->dtend( $repeat->getUnixEndTime());
                                return;				
					...
					if ($timezone != $jtimezone){
						...
						if (!$repeat2){
							...
							if ($truestarttime == JevDate::strftime("%H:%M:%S",JevDate::mktime($repeat2->hup(),$repeat2->minup(),$repeat2->sup(), 0, 0, 0))){
								...
								// In this scenario we have no idea what the time should be unfortunately - use the original event time/date from the first non-exception repeat
								$truedtstart = JevDate::strftime("%Y-%m-%d %H:%M:%S",$repeat2->dtstart());
								$truedtend = JevDate::strftime("%Y-%m-%d %H:%M:%S",$repeat2->dtend());
	...
	private function getFirstRepeat($row, $allowexceptions=true){
		...
		$db = JFactory::getDBO();
		$query = "SELECT ev.*, rpt.*, rr.*, det.*, exc.ex_id, exc.startrepeat as exstartrepeat, exc.oldstartrepeat as exoldstartrepeat, exc.exception_type "
