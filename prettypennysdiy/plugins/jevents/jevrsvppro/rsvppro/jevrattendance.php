...
class JevRsvpAttendance
{
	...
	public function editAttendance(&$extraTabs, &$row, &$params)
	{
		...
		{
			$eventid = 0;
                    //  $rsvpdata->regopen = "0000-00-00 00:00:00";
                    //  $rsvpdata->regclose = "0000-00-00 00:00:00";
                    //  $rsvpdata->cancelclose = "0000-00-00 00:00:00";                        
		}
...
STYLE;
			...
			<div id="jevrsvpattend">
				...
				if (!$rsvpdata->allowregistration)
					echo "style='display:none'";
						...
						$dbtemplates = $db->loadObjectList('id');
						echo $db->getErrorMsg();

						if (count($templates) > 1 || (count($templates) == 1 && $templates[0] == "fieldssample.xml")  || count($dbtemplates) >0 )
						{
							?>
							<div class="rsvp_extrafields">
								...
								<div  class="control-group">
									...
									if (intval($rsvpdata->template) > 0 && array_key_exists($rsvpdata->template, $dbtemplates))
									{
										$style = "";
									}
									else
									{
										$style = "style='display:none' ";
                                                                                if (intval($rsvpdata->template) > 0 && !array_key_exists($rsvpdata->template, $dbtemplates))
                                                                                {
                                                                                        echo "<strong>".JText::_("RSVP_MISSING_SESSION_TEMPLATE")."<strong><br/>"; 
                                                                               }
									}
							...
							<div class=" radio btn-group">
								...
								<label for="custom_rsvp_allowchanges0" class="btn radio"><?php echo JText::_('JEV_NO'); ?>
									<input type="radio" name="custom_rsvp_allowchanges" id="custom_rsvp_allowchanges0" value="0" <?php echo $rsvpdata->allowchanges ? "" : "checked='checked'"; ?>  onclick="updateCancelClose(0);"/>
                                                ...
                                                $warning1 = "";
                                                if ($rsvpdata->id){
                                                    $warning = " onclick=\"if (!confirm('".JText::_("JEV_CHANGING_THIS_SETTING_WILL_DELETE_ANY_EXISTING_ATTENDEES", true)."')){jQuery('input[name=custom_rsvp_allrepeats]:not(:checked)').parent().trigger('click'); }; \" ";
                                                    $warning0 = $rsvpdata->allrepeats ? $warning : "";
                                                    $warning1 = $rsvpdata->allrepeats ? "" : $warning ;
                                                }
                                                    ?>
						<div class="rsvp_allrepeats control-group">
							...
							<div class=" radio btn-group">
								<label for="custom_rsvp_allrepeats1" class="btn radio"><?php echo JText::_('JEV_ALL_REPEATS'); ?>
									<input type="radio" <?php echo $warning1;?> name="custom_rsvp_allrepeats" id="custom_rsvp_allrepeats1" value="1" <?php echo $rsvpdata->allrepeats ? "checked='checked'" : ""; ?>/>
								</label>
								<label for="custom_rsvp_allrepeats0" class="btn-small radio "><?php echo JText::_('JEV_SPECIFIC_REPEATS'); ?>
									<input type="radio"  <?php echo $warning0;?> name="custom_rsvp_allrepeats" id="custom_rsvp_allrepeats0" value="0" <?php echo $rsvpdata->allrepeats ? "" : "checked='checked'"; ?>/>
								</label>
							</div>
						</div>
						<div class="rsvp_showattendees control-group">
							...
							<div class=" radio btn-group">
								...
								<?php } ?>
                                                                <label for="custom_rsvp_showattendees3" class="btn radio"><?php echo JText::_('JEV_ONLY_TO_ATTENDEES'); ?>
                                                                        <input type="radio"  name="custom_rsvp_showattendees" id="custom_rsvp_showattendees3" value="3" <?php echo $rsvpdata->showattendees == 3 ? "checked='checked'" : ""; ?>/>
                                                                </label>
							</div>
						</div>
						<?php
		...
		if ($this->params->get("invites", 0))
		{
			...
			?>>
						...
						<div class="controls">
							...
							<select onchange="ticketsEditorPlugin.insert('custom_rsvp_message','custom_rsvp_message_fields' )" id="custom_rsvp_message_fields">
								<option value="Select ...:">Select ...</option>
								<optgroup label="<?php echo JText::_("RSVP_EVENT_FIELDS", true); ?>" >		
                                                                    <option value="USERNAME"><?php echo JText::_("JEVR_USERNAME"); ?></option>
                                                                    <option value="NAME"><?php echo JText::_("RSVP_INVITEE_REAL_NAME"); ?></option>
                                                                    <option value="EMAIL"><?php echo JText::_("RSVP_INVITEE_EMAIL"); ?></option>
                                                                    <option value="EVENT"><?php echo JText::_("RSVP_EVENT_TITLE"); ?></option>
                                                                    <option value="DATE}%Y %m %d{/DATE"><?php echo JText::_("RSVP_INVITE_EVENT_DATE"); ?></option>
                                                                    <option value="TIME}%Y %m %d{/TIME"><?php echo JText::_("RSVP_EVENT_TIME"); ?></option>
                                                                    ...
                                                                    <option value="CUSTOM"><?php echo JText::_("RSVP_CUSTOM_FIELD_SUMMARY"); ?></option>						
                                                                    <option value="WAITINGMESSAGE"><?php echo JText::_("RSVP_WAITING_MESSAGE"); ?></option>						
                                                                    <option value="ATTENDSTATE"><?php echo JText::_("RSVP_ATTEND_STATE"); ?></option>						
                                                                    <option value="CONFIRMED"><?php echo JText::_("RSVP_CONFIRMED_STATE"); ?></option>						
                                                                    <option value="DIRECTATTENDLINK}linktext{/DIRECTATTENDLINK"><?php echo JText::_("RSVP_DIRECT_ATTEND"); ?></option>						
                                                                    <option value="DIRECTNOATTENDLINK}linktext{/DIRECTNOATTENDLINK"><?php echo JText::_("RSVP_DIRECT_NO_ATTEND"); ?></option>						
	...
	{
                // icomoon vs bootstrap icons
                $clockicon = JFactory::getApplication()->isAdmin()? "icon-clock":"icon-time";
                $rsvpparams = JComponentHelper::getParams("com_rsvppro");
		...
		<div class="regopendatewrapper">
			<fieldset><legend><?php echo JText::_('JEV_STARTREGISTRATION'); ?></legend>
				...
				<div style="position:relative;float:left;" class="regopentime input-append bootstrap-timepicker">
					...
					<input class="inputbox" type="text"  id="regopentime" size="8" readonly="readonly" value="<?php echo $regopentime; ?>" /><span class="add-on"><i class="<?php echo $clockicon;?>"></i></span>
		...
		<div class="regclosedatewrapper">
			<fieldset><legend><?php echo JText::_('JEV_ENDREGISTRATION'); ?></legend>
				...
				<div style="position:relative;float:left;" class="regclosetime input-append bootstrap-timepicker">
					...
					<input class="inputbox" type="text" id="regclosetime" size="8" readonly="readonly" value="<?php echo $regclosetime; ?>" /><span class="add-on"><i class="<?php echo $clockicon;?>"></i></span>
		...
		<div id='jevendcancel' style="display:<?php echo $rsvpdata->allowcancellation ? 'block' : 'none'; ?>">
			<fieldset><legend><?php echo JText::_('JEV_ENDCANCELLATIONS'); ?></legend>
				...
				<div style="position:relative;float:left;" class="input-append bootstrap-timepicker">
					...
					<input class="inputbox" type="text" id="cancelclosetime" size="8" value="<?php echo $cancelclosetime; ?>" /><span class="add-on"><i class="<?php echo $clockicon;?>"></i></span>
		...
		<script  type="text/javascript" >
			<?php
                        // use #jevents because on some templates using 'body' leads to the timepicker being outside the edit page.
                        $appendto = $rsvpparams->get("appendtimepickerto",true) ?  "#jevents" :  "body";
			if (!$params->get("com_calUseStdTime"))
			{
				$timeoptions = "{appendWidgetTo:'$appendto' ,showMeridian:false, minuteStep:5, defaultTime:'%s'}";
			}
			else
			{
				$timeoptions = "{appendWidgetTo:'$appendto' ,showMeridian:true, minuteStep:5, defaultTime:'%s'}";
			}
			?>
	...
	public function storeAttendance($event)
	{
		...
		$eventid = isset($evdetail->_customFields["rsvp_evid"]) ? intval($evdetail->_customFields["rsvp_evid"]) : 0;
		if ($eventid == 0)
		{
			$eventid = $event->ev_id;
		}
	...
	public function reinstateReminders($row){
		...
		if (isset($row->ev_id)){
			...
			$jevent = $row;
                ...
                $db = JFactory::getDbo();
                $sql = "SELECT * FROM #__jev_attendance WHERE ev_id=" . $jevent->ev_id();
                $db->setQuery($sql);
                $rsvpdata = $db->loadObject();
                if (!$rsvpdata)
                {
		...
		$sql = "UPDATE #__jev_reminders set sentmessage = 0 WHERE at_id=".$rsvpdata->id;
	...
	public function updateReminders($event)
	{
		...
		$this->jevrreminders->updateReminders($rsvpitem, $event);
	}
