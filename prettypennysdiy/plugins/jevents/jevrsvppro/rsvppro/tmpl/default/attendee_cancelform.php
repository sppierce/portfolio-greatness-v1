...
// Always close this div efore a return!
$html .= '<div id="templateid'.(isset($this->templateInfo )?$this->templateInfo->id : 0).'">';
...
if ($this->rsvpdata->allrepeats)
{
	...
	if ($now > $regclose)
	{
		echo $html . $this->loadTemplate("registrationsclosed") . $this->loadTemplate("emptyattendanceform")."</div>";
		return;
	}
	else if ($now < $regopen)
	{
		echo $html . $this->loadTemplate("registrationsnotopen") . $this->loadTemplate("emptyattendanceform")."</div>";
		return;
	}
}
...
if ($this->params->get("capacity",0) )
{
	...
	if ($this->rsvpdata->capacity>0) {
		if ($attendeeCount >= $this->rsvpdata->capacity)
		{
			...
			else
			{
				...
				else
				{
					...
					echo $html . $this->loadTemplate("emptyattendanceform")."</div>";
					return;
				}
			}
		}
		else
		{
			...
			$html .=  ($this->params->get('show_atd_count_detail', 1)) ? $this->loadTemplate("numberattending") : '';
	}
}
else {
