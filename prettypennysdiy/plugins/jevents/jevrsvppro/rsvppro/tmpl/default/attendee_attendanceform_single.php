...
$html .= '<form action="' . $this->link . '"  method="post"  name="updateattendance" id="updateattendance"  enctype="multipart/form-data"  '.$confirm.' class="'.$formclass.'" >';
...
if ($this->rsvpdata->template != "")
{
	...
	if (is_int($xmlfile) || file_exists($xmlfile))
	{
		...
		// do a second time in case the JevRsvpParameter has changed them
		...
		if ($params->getNumParams() > 0)
		{
			if ($params->isMultiAttendee())
			{
				...
				else 	if ($sessionparams->get("maxguesttabs",0)>0 && $this->rsvpdata->capacity==0){
				...
				if ($params->isMultiAttendee() )
				{
					...
					if ($capacity==1){
						...
						$addGuestButton = '
				<div class="button2-left '.$addguestclass.'"  id="addguest" '.$addguestyle.'>
					<div class="blank">
