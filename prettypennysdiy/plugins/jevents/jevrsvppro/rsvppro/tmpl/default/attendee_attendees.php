...
if (count($this->attendees) > 0)
{
	...
	if ($this->attendees && count($this->attendees) > 0)
	{
		...
		foreach ($this->attendees as $attendee)
		{
			...
			{
				$html .="<tr class='jevattendeerow row$k'><td $rowspan class='regprimary'><div class='jevattendee'>" . $name . "</div></td>";
				...
				{
					$html .='<td align="center" ' . $rowspan . ' class="regremove"><img src="' . JURI::root() . $pluginpath.'assets/Trash.png" onclick="if (confirm(\'' . JText::_("JEV_CANCEL_ATTENDEE_ARE_YOU_SURE", true) . '\')) cancelAttendance(' . $attendee->id . ');" style="height:16px;cursor:pointer" alt="cancel" /></td>';
				}
			}
			else {
				$html .="<tr class='jevattendeerow  row$k' ><td $rowspan  class='regprimary'><div class='jevattendee'>" . $name . "</div></td>\n";
			...
			if ($showstatus)
			{
				...
				$html .='<td align="center" class="regstatus"><img src="' . JURI::root() . $pluginpath.'assets/' . $img . '"  style="height:16px;" alt="' . $img . '" />';
			...
			if ($this->params->get("showtimestamp", 1))
			{
				$format = $this->params->get("timestampformat", "%Y-%m-%d %H:%M");
					$html .='<td align="center" ' . $rowspan . ' class="regtimestamp">' . strftime($format, strtotime($attendee->created)) . '</td>';
			}
					...
					foreach ($params as $param)
					{
						...
						if (is_array($param['value']) && $attendee->guestcount > 0)
						{
							...
							$val = stripslashes($val);
                                                        if (isset($param["isname"]) && $param["isname"]){
                                                            $thclass="regisname ".$param["classname"];
                                                        }
                                                        else {
                                                            $thclass=$param["classname"];
                                                        }
                                                        
							$html .='<td  class="'.$thclass.'">' . $val . '</td>';
						}
						else
						{
                                                        if (isset($param["isname"]) && $param["isname"]){
							...
							$html .='<td ' . $rowspan . ' class="'.$thclass.'">' . stripslashes($param['value']) . '</td>';
						}
					}
				}
				$html .="</tr>\n";
	...
	if (count($this->attendees) > 0)
	{
		...
		{
			$html .='<th class="regprimary">' . JText::_( 'JEV_ATTENDEE' ) . ' (' . $this->attendeeCount . '/' . $this->rsvpdata->capacity . ($this->rsvpdata->waitingcapacity > 0 ? '+' . $this->rsvpdata->waitingcapacity : '') . ')</th>';
		}
		else
		{
			$html .='<th class="regprimary">' . JText::_( 'JEV_ATTENDEE' ) . '</th>';
		}
		...
		{
			$html .='<th class="regremove">' . JText::_( 'JEV_CLICK_TO_REMOVE' ) . '</th>';
		}

		if ($showstatus)
		{
			$html .='<th class="regstatus">' . JText::_( 'JEV_ATTENDANCE_STATUS' ) . '</th>';
		}

		//if ($this->params->get("showtimestamp", 1)  || $this->params->get("shownonattendees", 0))
		if ($this->params->get("showtimestamp", 1) )
		{
			$html .='<th class="regtimestamp">' . JText::_( 'JEV_REGISTRATION_TIME' ) . '</th>';
		}
				...
				if (is_int($xmlfile) || file_exists($xmlfile) )	{
					...
					foreach ($params as $param)
					{
						...
						if ($param["capacity"] > 0 && isset($param["capacitycount"]))
						{
							...
							$html .='<th class="regcapacity '.$thclass.'">' . stripslashes(RsvpHelper::translate($param['label'])) . ' (' . $param["capacitycount"] . '/' . $param["capacity"] . ')</th>';
						}
						else
						{
                                                        if (isset($param["isname"]) && $param["isname"]){
							...
							$html .='<th class="'.$thclass.'">' . stripslashes(RsvpHelper::translate($param['label'])) . '</th>';
						}
					}
				}
			}
				...
				for (var i=0;i<colcount;i++){
					if (rowspans[i] && rowspans[i]>1){
						...
						newTD.html(rowspanvalues[i]);
                                                newTD.attr("class",rowspanclasses[i])
						if (i==0){
							tr.prepend(newTD);
						}
						else {
				...
				for (var i=0;i<tdelements.length;i++){
					...
					if (i==0){
						...
						rowspanvalues.push( td.html()+ " (*)");
                                                rowspanclasses.push( td.attr("class"));
					}
					else {
						//rowspanvalues.push(  td.html()+ " ");
						rowspanvalues.push( " ");
