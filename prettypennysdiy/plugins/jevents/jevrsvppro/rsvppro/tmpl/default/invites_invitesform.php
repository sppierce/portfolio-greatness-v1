...
if ($user->id==$this->row->created_by() || JEVHelper::isAdminUser($user) || JEVHelper::canDeleteEvent($this->row, $user)){
	...
	if ($this->jomsocial){
        ...
        // Can't use <a href="#'.JText::_("JEV_ADD_INVITEES", true).'"  because of smooth scrolling template bugs from some providers!
	$html .='
	<div class="button2-left"   style="margin-right:10px;">
		<div class="blank">
			<a href="#ADD_INVITEES" onclick="addInvitees();return false;"  title="'.JText::_("JEV_ADD_INVITEES").'"  style="padding:0px 5px;text-decoration:none">'.JText::_("JEV_ADD_INVITEES").'</a>
		</div>
	</div>	
	';
	if ($this->params->get("invitejoomlagroups",0) && isset($this->jugroups)){
		...
		<div class="button2-left"   style="margin-right:10px;">
			<div class="blank">
				<a href="#INVITE_GROUP" onclick="addInvitees();document.getElementById(\'jugroups\').style.display=\'block\' ;return false;"  title="'.JText::_( 'JEV_INVITE_GROUP' ).'"  style="padding:0px 5px;text-decoration:none">'.JText::_( 'JEV_INVITE_GROUP' ).'</a>
			</div>
		</div>
		';
	...
	if ($this->jomsocial){
		...
		<div class="button2-left"   style="margin-right:10px;">
			<div class="blank">
				<a href="#INVITE_FRIENDS" onclick="inviteFriends(\''.JURI::root().$pluginpath."invitefriends.php".'\', \''.$client.'\');return false;"  title="'.JText::_( 'JEV_INVITE_FRIENDS' ).'"  style="padding:0px 5px;text-decoration:none">'.JText::_( 'JEV_INVITE_FRIENDS' ).'</a>
			</div>
		</div>
		';
		...
		if ($this->jsgroups && count($this->jsgroups)>0){
			...
			<div class="button2-left"   style="margin-right:10px;">
				<div class="blank">
					<a href="#INVITE_GROUP_JS" onclick="addInvitees();document.getElementById(\'jsgroups\').style.display=\'block\' ;return false;"  title="'.JText::_( 'JEV_INVITE_GROUP_JS' ).'"  style="padding:0px 5px;text-decoration:none">'.JText::_( 'JEV_INVITE_GROUP_JS' ).'</a>
				</div>
			</div>
			';
	...
	if ($this->groupjive){
		...
		*/
		if ($this->cbgroups && count($this->cbgroups)>0){
			...
			$html .= "<select name='custom_jevuser[]' id='custom_jevuser_cbgroupselection' size='".(count($this->cbgroups)<6?count($this->cbgroups)+1:6)."' onchange='inviteCBGroup(\"".JURI::root().$pluginpath."invitefriends.php"."\",this.options[this.selectedIndex].value ,\"".$client."\")'>";
			$html .= "<option value='NONE' selected='selected'>".JText::_( 'JEV_SELECT_GROUP' )."</option>";
