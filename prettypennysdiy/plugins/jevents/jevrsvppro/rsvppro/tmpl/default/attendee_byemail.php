...
if ($this->params->get("attendemails", 0) && $user->id == 0)
{
	...
	$this->checkemail = "try {if (!jevrsvpRequiredFields.emailcheck()) {alert('" . JText::_("JEV_MISSING_EMAIL", true) . "');return false;}}  catch (e) {if (document.getElementById('jevattend_email').value=='') {alert('" . JText::_("JEV_MISSING_EMAIL", true) . "');return false}};";

	//$html .= '<table width="100%" class="paramlist admintable" cellspacing="1">';
	$onchange = "return false;";
	$rsvpMustBeLoggedIn = "";
	$attendeeAlreadyExists = "";
	...
	if ($this->emailaddress=="" && $this->params->get("attendemailscheck", 1)){
		...
		$checkURL = JRoute::_("index.php?option=com_rsvppro&ttoption=com_rsvppro&typeaheadtask=gwejson&file=finduser&rsvpid=".$this->rsvpdata->id."&rp_id=".$this->row->rp_id()."&token=". JSession::getFormToken(), false);
		...
		$attendee_exists_html = JText::_("JEV_ATTENDANCE_CANNOT_REGISTER_A_SECOND_TIME_USING_THE_SAME_EMAIL_ADDRESS");
		$attendeeAlreadyExists = "<div id='attendeeAlreadyExists' style='display:none'><strong>$attendee_exists_html </strong></div>";
	...
	if ($this->params->get("autoregister", 0)){
