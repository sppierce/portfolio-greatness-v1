...
// prepare the messages
	...
	foreach ($reminders as $reminder)
	{
		...
		$name = $reminder->name ? $reminder->name : $reminder->email_address;
				...
				// should we skip non-attendees
				$skipnotattending = $rsvpparams->get("skipnotattending", 0);

				// check if the attendee has made any payments
                                if ($attendee)
                                {
                                        //  Skip not-attending users
                                        if ($attendee->attendstate == 0)
					{
						echo "Skip reminder to  ", $name . " -  they have cancelled<br/>";
                                            continue;
                                        }
					...
					{
						//  Skip not-attending users
						if ($attendee->attendstate == 0 || $attendee . attendstate == 2)
						{
							echo "Skip reminder to  ", $name . " -  they are not attendings<br/>";
							continue;
						}
					}
					...
					JLoader::register('JevRsvpParameter', JPATH_ADMINISTRATOR . "/components/com_rsvppro/libraries/jevrsvpparameter.php");
                                        ...
                                        // Replace reminder name field if it is set!
                                        $tempparams = $tempparams->renderToBasicArray(null, $attendee);
                                        $inputname = false;
                                        ...
                                        if ($inputname){
                                            $reminder->name = $inputname;
                                        }                                                
                                }
				else if ($skipnotattending)
				{
					echo "Skip reminder to  ", $name . " -  they are not attending<br/>";
					continue;
				}
			}
		}
...
function parseMessage($message, $rsvpdata, $row, $iuser, $creator)
{
	...
	if (count($matches) == 2)
	{
		$Itemid = JRequest::getInt("Itemid");
                if ($Itemid == 0) {
                    $Itemid = $rsvpparams->get("reminder_itemid");
                }
