<?php
/**
 * copyright (C) 2009-2017 GWE Systems Ltd - All rights reserved
 */
// no direct access
...
class plgJEventsJevrsvppro extends JPlugin {
	...
	function __construct(& $subject, $config) {
		...
		try {
			...
			$this->params->set("pluginversion", (string) $xml->version);
		} catch (Exception $ex) {
	...
	function onEditMenuItem(&$menudata, $value, $control_name, $name, $id, $param) {
		...
		if ((strpos($value, "jevrsvpsa:") === 0 && is_string($matchingextra2) && strpos($matchingextra2, $name) >= 0) || (($value == "" || $value == "0") && $matchingextra2 === false)) {
			...
			$options[] = JHtml::_('select.option', 'jevrsvpsa:2', JText::_('JEV_WHERE_EVENT_IS_OPEN_FOR_REGISTRATIONS'), 'id', 'title');
	...
	function onAfterSaveEvent($event, $dryrun = false) {
		...
		// if date/time or interval has changed then we need to update the existing reminders if they have already been sent
		$jevrattendance->reinstateReminders($event);
	}
	...
	function onListIcalEvents(& $extrafields, & $extratables, & $extrawhere, & $extrajoin, & $needsgroupdby = false, & $rptwhere = array()) {
		...
		for ($extra = 0; $extra < 20; $extra++) {
			...
			if (strpos($extraval, "jevrsvpsa:2") === 0) {
				$filters[] = "hidebasedonaccess";
				$filters[] = "hidebasedonopenclose";
				break;
			}
		}
	...
	public function onTranslateEvent(&$row, $language) {
		...
		$sql = "SELECT * FROM #__jev_attendance_translations WHERE atd_id = " . $rsvpdata->id . " and language=" . $db->quote($language);
		$db->setQuery($sql);
		$rsvpdataTranslation = $db->loadObject();

		$customfields = array(
			array("name" => "attendintro", "type" => "html", "label" => JText::_('JEV_ATTEND_INTRO')) ,
			/*array("name" => "subject", "type" => "text", "label" => JText::_('JEV_EMAIL_SUBJECT')),
			array("name" => "message", "type" => "html", "label" => JText::_('JEV_EMAIL_MESSAGE')),
			array("name" => "remindersubject", "type" => "text", "label" => "JEV_REMINDER_EMAIL_SUBJECT"),
			array("name" => "remindermessage", "type" => "html", "label" => JText::_('JEV_REMINDER_EMAIL_MESSAGE')),*/
			array("name" => "sessionaccessmessage", "type" => "text", "label" => JText::_('JEV_SESSION_NOACCESS_MESSAGE')));

		$user = JFactory::getUser();
		foreach ($customfields as $field) {
			$id = "rsvppro_" . $field["name"];
			$translation = new stdClass();
			$translation->label = "<strong>" . $field["label"] . "</strong>";
			$name = $field["name"];
			$translation->original = $field["type"] == "html" ? $rsvpdata->$name : nl2br($rsvpdata->$name);
			if (isset($rsvpdataTranslation->$name)) {
				$translation->translation = $rsvpdataTranslation->$name;
			} else {
				$translation->translation = "";
			}
			$fieldname = "translate_customfields[$id]";
			if ($field["type"] = "text") {
				$translation->translation = "<input type='text' value='$translation->translation' name='$fieldname' />";
			} else {
				$translation->translation = "<textarea rows='5' cols='40'   name='$fieldname' >" . $translation->translation . "</textarea>";
			}
	...
	public function onSaveTranslation($translationData) {
		...
		$eventid = intval($translationData["ev_id"]);
		...
		$sql = "SELECT * FROM #__jev_attendance_translations WHERE atd_id = " . $rsvpdata->id . " and language=" . $db->quote($translationData["trans_language"]);
		$db->setQuery($sql);
		$rsvpdataTranslation = $db->loadObject();
		...
		foreach ($customfields as $field) {
			$id = "rsvppro_" . $field["name"];
			if (isset($translationData["translate_customfields"][$id])) {
				$needToSave = true;
				$saveTranslation[$field["name"]] = $db->quote($translationData["translate_customfields"][$id]);
			}
			else {
				$saveTranslation[$field["name"]] = "''";
			}
		}
		if (count($saveTranslation)) {
			if ($rsvpdataTranslation){

				$sql = 'REPLACE INTO #__jev_attendance_translations (`id`, `atd_id`, `language`, `attendintro`,`subject`, `message`, `remindersubject`, `remindermessage`, `sessionaccessmessage`)
                            VALUES ( '.$rsvpdataTranslation->id.', '.$rsvpdataTranslation->atd_id.','. $db->quote($translationData["trans_language"]).','. implode(',',$saveTranslation). ')';
			}
			else {
				$sql = 'INSERT INTO #__jev_attendance_translations (`atd_id`,  `language`, `attendintro`,`subject`, `message`, `remindersubject`, `remindermessage`, `sessionaccessmessage`)
                            VALUES ( '.$rsvpdata->id.','. $db->quote($translationData["trans_language"]).','. implode(',',$saveTranslation). ')';
			}
		}
		$db->setQuery($sql);
		$db->execute();
	...
	static function fieldNameArray($layout = 'detail') {
		...
		if ($layout == "detail") {
			...
			$labels[] = JText::_("JEV_ADD_ATTENDEE_BTN", true);
			$values[] = 'ADD_ATTENDEE_BTN';
		}
		...
		if ($layout == "detail" || $params->get("showattendeecountinlists", 0)) {
			...
			$labels[] = JText::_("JEV_HAS_SPACES", true);
			$values[] = "HASSPACES";
	...
	static function substitutefield($row, $code) {
		...
		if ($code == "ATTENDSPACE" || $code == "HASSPACES") {
		...
		if ($code == "ATTENDWAIT") {
			if (isset($row->rsvpdata) && isset($row->rsvpdata->waitingcapacity) && $row->rsvpdata->waitingcapacity > 0)
				return $row->rsvpdata->waitingcapacity;
			else
				return 0;
		}
		...
		if ($code == "REGOPEN" || $code == "REGCLOSED" || $code == "REGOPENING" || $code == "REGCLOSING") {
			...
			if (isset($row->_regopen) && $row->_regopen != "0000-00-00 00:00:00" && $row->_allowregistration) {
				...
				// otherwise the start of the repeat
					...
					// use toMySQL to pick up timezone effects
					$adjustedregcloseMySQL = $adjustedregclose->toMySQL();
					$adjustedregopenMySQL = $adjustedregopen->toMySQL();

					if ($code == "REGCLOSED") {
						if ($now > $adjustedregcloseMySQL) {
							return JText::_("JEV_REGCLOSED_MESSAGE");
						}
						return "";
					}
					if ($code == "REGCLOSING") {
						if ($now < $adjustedregcloseMySQL) {
							// local timezone!
							return JText::sprintf("JEV_REGCLOSING_MESSAGE", $adjustedregclose->toFormat(JText::_("JEV_REGCLOSING_FORMAT")));
						}
						return "";
					}

					if ($now < $adjustedregcloseMySQL && $now > $adjustedregopenMySQL) {
						if ($code == "REGOPEN") {
							return JText::_("JEV_REGOPEN_MESSAGE");
						} else {
							return "";
					...
					if ($code == "REGOPENING") {
						if ($now < $adjustedregopenMySQL) {
							return JText::sprintf("JEV_REGOPENING_MESSAGE", $adjustedregopen->toFormat(JText::_("JEV_REGOPENING_FORMAT")));
						}
					}
				}
			} else
		...
		if ($code == "TICKETLINK") {
			if (isset($row->ticketlink) && $row->ticketlink != "") {
				...
				$jevrattendees = new JevRsvpAttendees($pluginparams, false , $rsvpdata);
				$jevrattendees->view->setLayout("attendee");
				// For sites that want tickets in list views
				if (!isset($row->rsvp_attendanceform) && JRequest::getCmd('task') != "icalrepeat.detail")
				{
					if (!isset($row->attendee)){
						$emailaddress = $jevrattendees->getEmailAddress("em2");
						if (!$emailaddress)
						{
							$emailaddress = $jevrattendees->getEmailAddress("em");
						}
						$attendee = $jevrattendees->isAttending($row->rsvpdata, $row, $emailaddress, true);
						$attending = false;
						if ($attendee)
						{
							$attending = true;
					...
					$jevrattendees->attendee = $row->attendee;

					if (JRequest::getCmd("task")!="accounts.paymentpage"){
						$jevrattendees->view->assignRef("attendee", $row->attendee);
						$jevrattendees->view->assignRef("rsvpdata", $row->rsvpdata);
						$jevrattendees->view->assignRef("row", $row);
						// ticket template uses a static for this !
						unset($jevrattendees->view->templateInfo);
						$row->ticketlink = $jevrattendees->view->loadTemplate("ticket");
						return $row->ticketlink;
					}
				}
			}
		...
		if ($code == 'ADD_ATTENDEE_BTN' && RsvpHelper::canCreateSessions()) {

		    JHtml::_('behavior.modal', 'a.jevmodal');
			$jinput = JFactory::getApplication()->input;
			$itemid = $jinput->get('Itemid', 0);
			$rp_id  = $row->rsvpdata->allrepeats === '1' ? '0' : $row->rp_id;
			$link = 'index.php?option=com_rsvppro&task=attendees.edit&atd_id[]=' . $row->rsvpdata->id . '|' . $rp_id  . '&rp_id=' . $row->rp_id . '&Itemid=' . $itemid . '&tmpl=component&' . JSession::getFormToken() . '=1';
			$link = JRoute::_($link);
			return "<a href='$link' title='" . htmlspecialchars(JText::_("RSVP_ADD_ATTENDEE_BTN_LINK")) . "' class=\"btn jevmodal\" rel=\"{handler: 'iframe', size: {x:600, y:500}}\" >" . JText::_('RSVP_ADD_ATTENDEE_BTN_LINK') . "</a>";
		}
