...
class JevCfParameter extends JRegistry
{
	...
	function __construct($data, $path = '', $event)
	{
		...
		// for attendees only fields
		$rsvpplugin = JPluginHelper::getPlugin("jevents", "jevcfsvppro");
...
(function ($){
	...
	if (form){		
		$(form).on('submit',function(event){if (!JevrRequiredFields.verify(form)) {event.preventDefault();event.stopImmediatePropagation();}});
		//JevrRequiredFields
	};
    });
})(jQuery);
...
SCRIPT;
	...
	public function renderToBasicArray($name = 'params', $group = '_default')
	{
		...
		foreach ($this->_xml[$group]->children() as $node)
		{
			...
			$result['access'] = ($this->attribute()->access != null) ? $this->attribute()->access : 1 ;
