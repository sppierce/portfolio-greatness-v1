...
class JFormFieldJevcfpaidorfree extends JevcfField
{
	/**
	* Element name
	*
	...
	*/
	...
	function getFilterInput($name, $value, &$node, $control_name)
	{
		...
		if (isset($this->event) && $this->event->ev_id()>0){
			$orders = $this->getOrders((int) $this->attribute("vmproductid",0), $this->event->ev_id());
			if (count($orders)==0){
	...
	public 	function onAfterSaveEvent($node, $value, $event)
	{
		...
		if ($value ==1 && !JFactory::getApplication()->isAdmin()){
			...
			$product = $this->getProduct((int) $this->attribute("vmproductid",0));

			$cart = $this->getCart();

			$category = (int) $this->attribute("vmcatid",0);
			if ($this->addToCart($cart, $product, $category, $event->ev_id)){

				$shoplink = JRoute::_("index.php?page=checkout.index&ssl_redirect=1&option=com_virtuemart". "&Itemid=" . (int) $this->attribute("menuid",1));
	...
	private function addToCart($cart, $product, $category, $event_id)
	{
		...
		{
			if ($entry["product_id"] == (int) $product->product_id && strpos($entry["description"], "EventId:" .$event_id)===0)
			{
				$matched = true;
			}
		}
		...
		if (!$matched)
		{
			...
			$entry["parent_id"] = (int) $product->product_id;
