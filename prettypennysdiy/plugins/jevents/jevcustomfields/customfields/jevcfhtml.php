...
<div class='jevcffieldinput'>
	...
	<div class="jevcfinputs">
		<textarea name="dv[<?php echo $id;?>]" id="dv<?php echo $id;?>" onchange="jevcftextarea.setvalue('<?php echo $id;?>');" onkeyup="jevcftextarea.setvalue('<?php echo $id;?>');"
                          rows="<?php echo $field?$field->attribute('rows'):5;?>" cols="<?php echo $field?$field->attribute('cols'):20;?>" ><?php echo $field?  htmlspecialchars_decode($field->attribute('default')):"";?></textarea>
	</div>
	<div class="jevcfclear"></div>

	<?php

	CustomFieldsHelper::editorbuttons($id, $field);
        CustomFieldsHelper::booleanField($field,"allowhtml[$id]", "allowhtml0$id", "allowhtml2$id", JText::_("JEVCF_ALLOW_HTML_CONTENT"), "allowhtml", JText::_("JEVCF_ALLOW_HTML_CONTENT_DESC"),"","", 1);
...
<div class='jevcffieldpreview'  id='<?php echo $id;?>preview'>
	<div class="previewlabel"><?php echo JText::_("JEVCF_PREVIEW");?> : <?php echo JText::_("JEVCF_EDITOR_NOT_PREVIEWED");?></div>
...
<div class="jevcfclear"></div>
	...
	function fetchRequiredScript($name, &$node, $control_name) 
		...
		//return "JevStdRequiredFields.fields.push({'name':'".$control_name.$name."', 'default' :'".$this->attribute('default') ."' ,'preAction' :'". addslashes($editor->save($control_name.$name)) ."; if (tinyMCE) {alert(\'setactive\');tinyMCE.EditorManager.setActive(tinymce.get(\'".$control_name.$name."\'));}','getValue' :'".addslashes($editor->getcontent($control_name.$name)) ."' ,'reqmsg':'".trim(JText::_($this->attribute('requiredmessage'),true))."'}); ";
                return "JevStdRequiredFields.fields.push({'name':'".$control_name.$name."', 'default' :'".$this->attribute('default') ."' ,'preAction' : 'if (tinyMCE) {tinyMCE.EditorManager.setActive(tinyMCE.get(\'".$node->name."\'));}','getValue' :'".addslashes($editor->getcontent($name)) ."' ,'reqmsg':'".trim(JText::_($this->attribute('requiredmessage'),true))."'}); ";
	}
	...
	public function toXML($field){
		...
		foreach (get_object_vars($field) as $k => $v){
			...
			if ($k == "params")
			{
				...
				if (is_object($field->params))
				{
					...
					{
						$result[] = $label . '="' . htmlspecialchars($value) . '" ';
					}
				}
				continue;
			}                        
        ...
        public function getInput() {
            ...
            if ($this->value == $this->attribute('default')) {
                $this->value = htmlspecialchars_decode($this->value);
            }
