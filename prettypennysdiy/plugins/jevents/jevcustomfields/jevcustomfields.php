...
JLoader::register('JevCfForm', __DIR__ . "/customfields/jevcfform.php");
...
class plgJEventsJevcustomfields extends JPlugin
{
	...
	public static $cachedCfData = array();
	public static $cachedParamsData = array();
				...
				if (($matchingextra == $id && strpos($value, "jevcf:") === 0) || (($value == "" || $value == "0") && $matchingextra === false))
				{
					...
					foreach ($invalue as $inv)
					{
						$values[$inv->id] = $inv->val;
                                                $ignores[$inv->id] = isset($inv->ignore) ? $inv->ignore : 0;
					}
...
var JevrCustomFields = {
	...
	convert:function (){
		...
		JevrCustomFields.fields.forEach(function(el){
                        var elem = jQuery("[name="+el+"]");
			...
			if (!elem.length){
                        ...
                        if (elem.attr('type') == "checkbox" || elem.attr('type') == "radio"  || elem.attr('type') == "select-=ne" || elem.attr('type') == "select-multiple" ) {
				// could be a radio box!
				jQuery('input[name='+el+']').each(function(index, item){
					if (item.checked) {
						var id = el;
						var val = item.value;
                                                var ignore = jQuery("#"+el+"_ignore").prop('checked') ? 1 : 0;
						values.push({'id':id, 'val':val, 'ignore':ignore});
					}
				});
				// could be a select box!
				jQuery('select[name='+el+'] option').each(function(index, item){
					if (item.selected) {
						var id = el;
						var val = item.value;
                                                var ignore = jQuery("#"+el+"_ignore").prop('checked') ? 1 : 0;
		...
		jQuery('#paramsextras$id').val( "jevcf:"+$json(values));
	}
};
SCRIPT;

					$script .= "jQuery(window).load(function(){";
					$html = "<div id='frogswerehere' >";
					$firstpass = true;
					foreach ($nodes as $node)
					{
						...
						if (array_key_exists("$elemname", $ignores))
						{
							$ignore = $ignores["$elemname"]? " checked='checked'": "";
						}
						else
						{
							$ignore = "";
						...
						$formelement = str_replace($node->name, $elemname,$formelement);
                                                $formelement .= "<label for='$elemname"."_ignore"."' style='display:inline;margin-left:10px;'><input type='checkbox' $ignore value='1' id='$elemname"."_ignore"."'>".JText::_("JEVCF_IGNORE_FILTER")."</label>";
						$html .= "<div class='control-group'>\n";
						$html .= "<div class='control-label'><label'>$label</label></div><div class='controls'>$formelement</div>\n";
						$html .= "</div>";
						if ($firstpass){
							$script .= "\n jQuery('#frogswerehere').on('mouseout',function(item){JevrCustomFields.convert();});\n";
							$firstpass = false;
						}
	...
	public function onEventEdit(&$extraTabs, &$row, &$jevparams)
	{
		$html = "";
				...
				foreach ($groups as $group => $element)
				{
					...
					if ($count > 0 && $group != "default")
					{
						...
						if (count($customfields) > 0)
						{
							...
							{
                                                            if ($customfields[$key]["label"]=="" && $customfields[$key]["input"]==""){
                                                                continue;
                                                            }
                                                            $datashowon = '';
                                                            $field = $customfields[$key]["node"];
                                                            $showonstring = $field->getAttribute('showon');
                                                            if (!$showonstring && $field->getAttribute('cf') && $field->getAttribute('cfvfv')!="")
                                                            {
                                                                // Drop the 'field' prefix from our field id
                                                                $showonstring = "jform[".substr($field->getAttribute('cf'), 5)."]:".$field->getAttribute('cfvfv');
                                                            }
                                                            if ($showonstring)
                                                            {
                                                            ...
                                                            ?>
                                                            <div class="row jevplugin_<?php echo $key; ?>" <?php echo $datashowon; ?> >
                                                                    <div class="span2">
                                                                        <?php echo $customfields[$key]["label"]; ?>
                                                                    </div>
                                                                    <div class=" span10">
		...
		if ($template != "")
		{
			...
			if (file_exists($xmlfile))
			{
				...
				foreach ($params as $param)
				{
					...
					if (!isset($evdetail->_customFields[$param["name"]]) )
					{
						...
						if (!$parentEventDetail)
						{
							...
							$parentEventDetail = (int) $db->loadResult();
				...
				foreach ($params as $param)
				{
					...
					JTable::addIncludePath(__DIR__ . "/customfields/");
					$cfitem =  JTable::getInstance('jevcustomfields');
					$cfitem->value = $customfield;
					$cfitem->evdet_id = (int) $eventid;
					$cfitem->name = $param["name"];
					$cfitem->id = $id;
                                        $cfitem->language = "*";
                                        
                                        // $translationData["trans_language"];
	...
	public function onStoreCustomEvent($event)
	{
		return true;
	...
	public function onAfterSaveEvent($event, $dryrun = false)
	{
		if ($dryrun)
		{
			return true;
			...
			if (file_exists($xmlfile))
			{
				...
				foreach ($groups as $group => $element)
				{
					...
					foreach ($groupparams as $p => $node)
					{
						...
						{
							$node->onAfterSaveEvent($node, $value, $event);
						}
					}
				}
			}
	 ...
	 */
	public function onDeleteEventDetails($idlist)
	{
	...
	public function onDeleteCustomEvent($idlist)
	{
...
*/
	...
	public function onListIcalEvents(& $extrafields, & $extratables, & $extrawhere, & $extrajoin, & $needsgroupdby = false)
	{
		...
		if (!isset($usefilter))
		{
			...
			{
				$extratest = $compparams->get("extras" . $extra, false);
				if (strpos($extratest, "jevcf:") === 0)
				{
					$extraval = $extratest;
					break;
				}
			...
			$menuModuleFilters = array();
                        $oldIndexedVisibleFilters = $registry->get("indexedvisiblefilters", false);
			...
			if ($extraval)
			{
				...
				if ($invalue != "")
				{
					if (strpos("&", $invalue)!==false && strpos(";", $invalue)!==false )
					{
						// assumes the data was stored in json encoded format
						$invalue = htmlspecialchars_decode($invalue);
					}
					// assumes the data was stored in json encoded format
					$invalue = json_decode($invalue);					
				}
				else
				{
					$invalue = array();
				...
				foreach ($invalue as $inv)
				{
                                    ...
                                    if (isset($inv->ignore) && $inv->ignore) {
                                        continue;
                                    }
					$skipreset = true;
					...
					if (isset($cfnodes) && isset($cfnodes[$fieldname]) && $inv->val==$cfnodes[$fieldname]->filterdefault){
						continue;
                                        ...
                                        $menuModuleFilters[] = $fieldname;
					$oldRequestValues[$fieldname . "_fv"] = JRequest::getVar($fieldname . "_fv", "not set");
					// overwrite here since we are handing menu item or module parameters!
					JRequest::setVar($fieldname . "_fv", $inv->val, "method", true);
				}
				...
				$registry->set("indexedvisiblefilters", $indexedvisiblefilters);
                        ...
                        if (count($menuModuleFilters) || (in_array("customfield", $oldIndexedVisibleFilters))) {
                            ...
                            // Set the custom field filter to NOT ignore the filter module temporarily
                            // but ONLY if there are any menu or module filters to apply
                            $ignorefiltermodule = $compparams->get("ignorefiltermodule", false);
                            if (count($menuModuleFilters)){
                                $compparams->set("ignorefiltermodule", false);
                            }
                            ...
                            // Restore the ignore the filter module setting
                            $compparams->set("ignorefiltermodule", $ignorefiltermodule);
                            //$filters->setSearchKeywords($extrawhere, $extrajoin);
                            $registry->set("jevents.menuModuleFilters",false);
                        }
			...
			foreach ($oldRequestValues as $key => $val)
			{
				...
				else
				{
					...
					JFactory::getApplication()->setUserState( $key."_ses",$val);
                        ...
                        $registry->set("indexedvisiblefilters", $oldIndexedVisibleFilters);
                        
                        // Now must reapply the custom fields filters to handle the URL and session values - the above was for menu item and module settings
                        ...
                        if ( in_array("customfield", $oldIndexedVisibleFilters) &&  !$ignorefiltermodule && count($menuModuleFilters)>0){
                            $registry->set("jevents.customFieldMapPrefix","url");
                            $filters = jevFilterProcessing::getInstance(array("Customfield"), JPATH_SITE . "/plugins/jevents/filters", false, "url".$moduleid);
                            $filters->setWhereJoin($extrawhere, $extrajoin);
                            $registry->set("jevents.customFieldMapPrefix","");
                        }
	...
	public function onSearchEvents(& $extrasearchfields, & $extrajoin, & $needsgroupdby = false)
	{
		static $usefilter;
	...
	public function onListEventsById(& $extrafields, & $extratables, & $extrawhere, & $extrajoin, & $needsgroupdby = false)
	{
	...
	public function onDisplayCustomFields(&$row)
	{
		...
		if ($template != "")
		{
			...
			if (file_exists($xmlfile))
			{
				...
				$customdata = $this->getCachedCfData($id, $user->id, "*");
                                $translatedcustomdata = $this->getCachedCfData($id, $user->id, JFactory::getLanguage()->getTag());
                                foreach ($customdata as $key => $value){
                                    if (!array_key_exists($key, $translatedcustomdata)){
                                        $translatedcustomdata[$key] = $value;
                                    }
                                }
				$params = JevCfForm::getInstance("com_jevent.customfields", $xmlfile, array('control' => 'jform', 'load_data' => true), true, "/form");
				$params->bind($translatedcustomdata);
				$params->setEvent($row);
				$customfields = array();
		...
		foreach (array_keys($customfields) as $id)
		{
			...
			if (isset($customfield["readaccess"]) && !empty($customfield["readaccess"]))
			{
	...
	public function onDisplayCustomFieldsMultiRow(&$rows)
	{
		...
		if ($template != "")
		{
			...
			if (file_exists($xmlfile))
			{
				...
				$untranslatedCustomdata = $this->getCachedCfData($ids, $user->id, "*");
				if (is_null($untranslatedCustomdata) || count($untranslatedCustomdata) == 0)
				{
					return;
				}
                                
                                $customdata = $this->getCachedCfData($ids, $user->id, JFactory::getLanguage()->getTag());
                                if (is_null($customdata)) {
                                    $customdata = array();
                                }
                                foreach ($untranslatedCustomdata as $evdet_id => $evcfdata){
                                    // translation doesn't exist but we have an original 
                                    if (!array_key_exists($evdet_id, $customdata)){
                                        $customdata[$evdet_id] = $evcfdata;
                                        continue;
                                    }
                                    foreach ($evcfdata as $key => $val){
                                        if (!array_key_exists($key, $customdata[$evdet_id])){
                                            $customdata[$evdet_id][$key] = $val;
                                        }
                                    }
                                }                                
		...
		foreach ($rows as &$row)
		{
			...
			else
			{
				...
				$tempdata = array();
                                if (isset($customdata[$row->evdet_id()])){
                                    foreach ($customdata[$row->evdet_id()] as $rowdata)
                                    {
                                            if ($needsSeparateRepeats && array_key_exists($rowdata->name,$needsSeparateRepeatFields))
                                            {
                                                    if ($rowdata->value == $row->rp_id())
                                                    {
                                                            // must eliminate comma separated names
                                                            $tempdata[str_replace(".", "_", $rowdata->name)] = $rowdata->value;
                                                    }
                                            }
                                            else if ($rowdata->evdet_id == $row->evdet_id())
                                            {
                                                    // must eliminate comma separated names
                                                    $tempdata[str_replace(".", "_", $rowdata->name)] = $rowdata->value;
                                            }
	...
	public function onTranslateEvent(&$row, $language)
	{

                // Dummy usage - to stop javascript error messages
		...
		if ($template != "")
		{
			...
			if (file_exists($xmlfile))
			{
				...
				$user = JFactory::getUser();
				$customdata = $this->getCachedCfData($id, $user->id);
                                $translatedCustomdata = $this->getCachedCfData($id, $user->id, $language);
                        
                                foreach ($customdata as $key => $translation) {
                                    if (!isset($translatedCustomdata[$key])) {
                                        $translatedCustomdata[$key] = $translation;
                                    }
                                }
                                
				$params = JevCfForm::getInstance("com_jevent.customfields", $xmlfile, array('control' => 'translate_customfields', 'load_data' => true), true, "/form");
				$params->bind($customdata);
				$params->setEvent($row);
				$customfields = array();

                                $translatedParams = JevCfForm::getInstance("com_jevent.customfields.translation", $xmlfile, array('control' => 'translate_customfields', 'load_data' => true), true, "/form");
				$translatedParams->bind($translatedCustomdata);
				$translatedParams->setEvent($row);
				$translatedCustomfields = array();

                                $translatedCustomfieldInputs = array();
                                
				// Slimmer method since we don't need the elements
				$groups = $translatedParams->getFieldsetsBasic();
				foreach ($groups as $group => $elementNotNeeded)
				{
					$extracustomfields = $translatedParams->renderToBasicArray('params', $group);
					if ($extracustomfields) $translatedCustomfields = array_merge($translatedCustomfields,$extracustomfields );
                                        
                                        $extraCustomfieldInputs = array();
                                        $translatedParams->render('custom_', $group, $extraCustomfieldInputs);
                                        if ($extraCustomfieldInputs) $translatedCustomfieldInputs = array_merge($translatedCustomfieldInputs,$extraCustomfieldInputs );
                                        
                                }

                                // Do output of current values second since some custom fields use static variables during input
                                $customfieldInputs = array();
				...
				foreach ($groups as $group => $elementNotNeeded)
				{
					...
					if ($extracustomfields) $customfields = array_merge($customfields,$extracustomfields );
                                        ...
                                        $extraCustomfieldInputs = array();
                                        $params->render('custom_', $group, $extraCustomfieldInputs);
                                        if ($extraCustomfieldInputs) $customfieldInputs = array_merge($customfieldInputs,$extraCustomfieldInputs );
		...
		$row->customfields = $customfields;                
                $row->customfieldInputs = $customfieldInputs;
                $row->translatedCustomfieldInputs = $translatedCustomfieldInputs;
                if (!isset($row->customfieldTranslations)){
                    $row->customfieldTranslations = array();
                }
		...
		{
			$customfield = $customfields[$id];
                        // No need to check access for translation!

                        // Ignore hidden value too!
                        
                        $translation = new stdClass();
                        $translation->label = $customfield["label"];
                        $translation->original = $customfield["fieldtype"]=="jevcfhtml"?$customfield["value"]:nl2br($customfield["value"]);
                        if (isset($row->translatedCustomfieldInputs["customfield_".$id])) {
                            $translation->translation = $row->translatedCustomfieldInputs["customfield_".$id]["input"];
                        }
                        else if (isset($row->customfieldInputs["customfield_".$id])) {
                            $translation->translation = $row->customfieldInputs["customfield_".$id]["input"];
                        }
                        else {
                            $translation->translation = "";                          
                        }
                        
                        $row->customfieldTranslations[$id] = $translation;
	...
	public function onSaveTranslation($translationData)
        {
            if (!array_key_exists("translate_customfields", $translationData)){
                return true;
            }
            // New parameterised fields
            $hasparams = false;
            ...
            if ($template != "")
            {
                    ...
                    if (file_exists($xmlfile))
                    {
                            ...
                            $evdet_id = $translationData["trans_evdet_id"];
                            $id = $evdet_id;
                            $customdata = $this->getCachedCfData($id, -1, "*");
                            $translatedCustomdata = $this->getCachedCfData($id, -1, $translationData["trans_language"]);
                            ...
                            foreach ($params as $param)
                            {
                                    ...
                                    if (!array_key_exists($param["name"], $translationData["translate_customfields"]))
                                    {
                                       echo "Delete any missing translations";
                                    }                                        
                                    
                                    if (!is_array($translationData["translate_customfields"][$param["name"]]))
                                    {
                                            if ($param["allowraw"])
                                            {
                                                    $customfield = $translationData["translate_customfields"][$param["name"]];
                                            }
                                            else if ($param["allowhtml"])
                                            {
                                                    static $safeHtmlFilter;
                                                    ...
                                                    $customfield = $safeHtmlFilter->clean($translationData["translate_customfields"][$param["name"]]);
                                            }
                                            else
                                            {
                                                    static $noHtmlFilter;
                                                    ...
                                                    $customfield = $noHtmlFilter->clean($translationData["translate_customfields"][$param["name"]]);
                                            }
                                    }
                                    else if (isset($translationData["translate_customfields"][$param["name"]]))
                                    {
                                            $customfield = implode(",", $translationData["translate_customfields"][$param["name"]]);
                                    }
                                    else {
                                            $x =1;
                                    }
                                    ...
                                    // is this an update?
                                    // this no longer works because we do not pass through the 
                                    if (isset($translatedCustomdata["fieldid_".$param["name"]]) &&  $translatedCustomdata["fieldid_".$param["name"]] > 0)
                                    {
                                            $transid = $translatedCustomdata["fieldid_".$param["name"]];
                                            
                                            // Do we need to remove a translation which is the same as the original 
                                            if ($translatedCustomdata[$param["name"]]==$customdata[$param["name"]]) {
                                                JTable::addIncludePath(__DIR__ . "/customfields/");                                                
                                                $cfitem =  JTable::getInstance('jevcustomfields');                                                    
                                                $cfitem->delete($transid);
                                            }
                                    }
                                    
                                    // if a null translation then skip!
                                    if ($translationData["translate_customfields"][$param["name"]] == $customdata[$param["name"]]) {
                                        // we have a tanslation to remove first!
                                        if ($transid>0){
                                            JTable::addIncludePath(__DIR__ . "/customfields/");                                                
                                            $cfitem =  JTable::getInstance('jevcustomfields');                                                    
                                            $cfitem->delete($transid);                                            
                                    ...
                                    $cfitem =  JTable::getInstance('jevcustomfields');
                                    $cfitem->value = $customfield;
                                    $cfitem->evdet_id = (int) $evdet_id;
                                    $cfitem->name = $param["name"];
                                    $cfitem->id = $transid;
                                    $cfitem->language = $translationData["trans_language"];
                                    $cfitem->store();
				...
				if (count($customfields) > 0)
				{
					foreach ($customfields as $customfield)
					{
						...
						if (isset($customfield["fieldnamearray"]) && count($customfield["fieldnamearray"])>0){
							...
							$cf_labels_cnt = count($customfield["fieldnamearray"]["labels"]);

							for ($i=0; $i < $cf_labels_cnt; $i++ ) {
					...
					// Raw values at the end since they are rarely used
					foreach ($customfields as $customfield) {
						$raw = JText::_("JEV_CUSTOM_FIELD_RAW");
						if (strpos($raw, '%') === false)
						{
							$raw = "%s Raw";
						}
						$labels[] = JText::sprintf($raw, str_replace(":", "", $customfield["label"]));
						$values[] = $customfield["name"] . "_raw";
					}
				}
			}
		}
	...
	public static function substitutefield($row, $code)
	{
		...
		if (strlen($code) > 4 && strrpos($code, "_raw") == strlen($code) - 4)
		{
			$code = substr($code, 0, strlen($code) - 4);
			if (isset($row->customfields) && array_key_exists($code, $row->customfields))
			{
				...
				return isset($row->customfields[$code]["rawvalue"]) ? $row->customfields[$code]["rawvalue"] : "";
			}
		}
	...
	{
		$xmlfile = __DIR__ . "/customfields/templates/" . $template;
		return $xmlfile;
	...
	private function getCachedCfData($evid, $userid = -1, $language="*")
	{
		$id = $evid;
		$multievent = false;
                $idsuffix = "";
		...
		if (!array_key_exists($id, self::$cachedCfData))
		{
			...
			if ($userid!=-1){
				...
				$db->setQuery($sql = "SELECT * FROM #__jev_customfields WHERE evdet_id IN (" . $evid . ") AND (user_id=0 OR user_id=" . $userid . ") and language=".$db->quote($language)." ORDER BY user_id ASC ");				
			}
			else {
				$db->setQuery("SELECT * FROM #__jev_customfields WHERE evdet_id=" . $evid." and language=".$db->quote($language));
			}
			if ($multievent){
				$cfdata  = $db->loadObjectList();
                                ...
                                foreach($cfdata as $dataelem){
                                    if (!isset($data[$dataelem->evdet_id])){
                                        $data[$dataelem->evdet_id] = array();
                                    }
                                    if (strpos($dataelem->name, ".") !== false)
                                    {
                                            $dataelem->name = str_replace(".", "_", $dataelem->name);
                                    }
                                    $dataelem->fieldid = "fieldid_".$dataelem->name;
                                    $data[$dataelem->evdet_id][$dataelem->name] = $dataelem;
                                }
                                self::$cachedCfData[$id] = $data;
			}
			else {
	...
	public function onJEventsPluginController ($plugin, $task) {
		if ($plugin == "jev_customfields" && JFactory::getApplication()->isAdmin()) {
			$user      = JFactory::getUser();
			// Stop if user is not authorised to manage JEvents
			if (!$user->authorise('core.admin',      'com_jevents')) {
				return;
			}

			include_once (JPATH_SITE."/plugins/jevents/jevcustomfields/customfields/controller.php");
			$controller = new JevCustomFieldsController();
