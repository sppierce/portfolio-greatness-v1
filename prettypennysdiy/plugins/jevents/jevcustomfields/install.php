...
class plgjeventsjevcustomfieldsInstallerScript
{
	...
	public function preflight($type, $parent) {
        ...
        if (version_compare(PHP_VERSION, "5.4", "<")){
                Jerror::raiseWarning ( null, JText::_('PLG_CUSTOM_FIELDS_MIN_PHP_VER'));
                return false;
...
// Joomla! broke the update call, so we have to create a workaround check.
        ...
        if ($is_enabled == 1) {
            ...
            if (version_compare( $app->version , '3.1.16', "lt")) {
	...
	function update($parent)
	{
		...
		{
			$sql = "ALTER TABLE #__jev_customfields ADD COLUMN language varchar(20) NOT NULL default '*'";
			$db->setQuery($sql);
			@$db->query();
		}
		...
		if (!array_key_exists("detval", $indexes))
		{
			...
			@$db->query();
                ...
                if (!array_key_exists("detlang", $indexes))
		{
			$sql = "ALTER TABLE `#__jev_customfields` ADD INDEX detlang (evdet_id,user_id, language(5))";
			$db->setQuery($sql);
			@$db->query();
		}
...
CREATE TABLE IF NOT EXISTS #__jev_customfields(
	...
	INDEX detlang (evdet_id,user_id, language(5))                                        
)  $charset;
SQL;
				$db->setQuery($sql);
				if (!$db->query())
        ...
        $sql = <<<SQL
CREATE TABLE IF NOT EXISTS #__jev_customfields_combined(
	id int(11) NOT NULL auto_increment,
	evdet_id int(11) NOT NULL default 0,
	user_id int(11) NOT NULL default 0,
	rp_id int(11) NOT NULL default 0,
        extension varchar(20) NOT NULL default 'com_jevents',
        language varchar(20) NOT NULL default '*',
