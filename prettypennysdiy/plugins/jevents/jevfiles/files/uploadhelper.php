...
class JevUploadHelper
{
	...
	function processFileUpload($file, $suffix="xml", $allowedfileExtensions = array("flv", "xml"))
	{
		...
		$uploadedFile = $files->get($file, false, "raw");
	...
	function processImageUpload($file)
	{
		...
		if ($imagew > 0 && $imageh > 0)
		{
			...
			$this->scaleImage($fname, $imagew, $imageh, $imagesize, $no_thumbanil, false);
                        ...
                        $this->error_goback(JText::_("JEV_PROBLEMS_SCALING_IMAGE")."\n".$e->getMessage());
                    }
		}
		...
		$thumbw = $thumbw>0 ? $thumbw : 100;
		$thumbh = $thumbh>0 ? $thumbh : 100;
	...
	function scaleImage($img, $maxwidth=150, $maxheight=150, $imagesize=10, $nothumbnail=1500000, $thumb=true)
	{
		...
		if ($thumb)
		{
			...
			if ($thumbWidth == $origw && $thumbHeight == $origh) {
				$dst_img = $src_img;
			}
			else {
			...
			$this->ensureDirExists($thumbdir);
                        ...
                        $quality = $this->params->get("imagequality", 90);
                        if ($info->type == IMAGETYPE_PNG && $quality >= 10) {
                            // PNG quality is up to 10
                            $quality = intval($quality/10);
                            $quality = $quality>9?9:$quality;
                        }
			if (!$dst_img->toFile($dest_file, $info->type, array("quality"=>$quality)))
			{
		...
		else
		{
			...
			if (!$dst_img->toFile($dest_file, $info->type, array("quality"=>$quality)))
			{
				$this->error_goback(JText::_("JEV_problems_scaling_image") . " 6 ");
				return false;
			}
