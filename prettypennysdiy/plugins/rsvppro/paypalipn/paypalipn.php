 ...
 */
	...
	//	$this->securityCheck();
		...
		if ( !$understoodResponse )
		{
			...
			//$this->PayPal_Log( $logText );
                        ...
                        try {
                            if ($secure){
                                $paypal_url = "https://$mode/cgi-bin/webscr";
                            }
                            else {
                                $paypal_url = "http://$mode/cgi-bin/webscr";
                            }
                            $logText .= "Trying Curl as fallback to fputs at $paypal_url \n";
                            
                            $ch = curl_init($paypal_url);
                            if ($ch == FALSE) {
                                $logText .= "Invalid PayPal Response from Curl!\n";                                
                                $logText .= date('[Y-m-d H:i e] '). "Can't connect to PayPal to validate IPN message: " . curl_error($ch);
                                $this->PayPal_Log( $logText );
                                $this->PayPal_Error( $logText  , "");                                                                    
                                curl_close($ch);
                                return;
                            ...
                            if ( strpos( $res, 'VERIFIED')  !== false ) {
                                $understoodResponse = true;
                                $logText  = "IPN Response - " . gmstrftime ("%b %d %Y %H:%M:%S", time()) . "\n";
                            }
                            if ( !$understoodResponse )
                            {                            
                                $logText .= "Bad Response from Curl !\n";
                                $logText .= "res = ".$res;
                                $this->PayPal_Log( $logText );
                                $this->PayPal_Error( $logText  , "");  
                                return;
                            ...
                            $logText .= "Caught Error from Curl";
                            $this->PayPal_Log( $logText );
                            $this->PayPal_Error( $logText  , "");                                    
	...
	private function notifyPaypalPayment($transaction, $attendee, $rsvpdata, $aftertransation=false){
		...
		if (((!$aftertransation && $templateParams->get("notifypplpay", 1)==2) || ($aftertransation && $templateParams->get("notifypplpay", 1)==1))  && $transaction->gateway == "paypalipn")
		{
			...
			$comparams->set("notifybcc", "staceyxnyc@yahoo.com");
