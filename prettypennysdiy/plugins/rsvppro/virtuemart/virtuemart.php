...
class plgRsvpproVirtuemart extends JPlugin
{
	...
	public function generatePaymentPage(&$html, $attendee, $rsvpdata, $event, &$transaction)
	{
		...
		$this->addToCart($tableproduct, $rawamount, $transaction, $rsvpdata, $detaillink);
	...
	{
                // needed for language processing
                if (!isset($this->rsvpdata)){
...
SQL;
	...
	private function addToCart($product, $amount, $transaction, $rsvpdata, $detaillink)
	{
		if (!class_exists('VmConfig'))
		...
		$vmcart->add(array(intval($product->virtuemart_product_id)), $success);
                ...
                if ($templateParams->get("vmredirectocart",0)){
                    $Itemid = JRequest::getInt("Itemid", 0);
                    JFactory::getApplication()->redirect(JRoute::_("index.php?view=cart&option=com_virtuemart&Itemid=$Itemid", false));
                }
                else {
                    JFactory::getApplication()->redirect($detaillink, JText::_("RSVP_PRODUCT_ADDED_TO_CART")) ;
                }
	...
	private function notifyVMPayment($transaction, $attendee, $rsvpdata){
        ...
        {
            	// Make sure the product hasn't been created yet
		$product = $this->getProduct($rsvpdata, $rpid, $eventid, $event);
