...
if (!class_exists('JOAuth2Client'))
{
	defined('JPATH_LIBRARIES_PWEB') or die;
	JLoader::import('joomla.oauth2.client', JPATH_LIBRARIES_PWEB);
	JLoader::register('JOAuth2Client', JPATH_LIBRARIES_PWEB.'/joomla/oauth2/client.php', true);

	if (version_compare(JVERSION, '2.5.15') == -1)
	{
		JLoader::import('joomla.http.factory', JPATH_LIBRARIES_PWEB);
		JLoader::register('JHttpFactory', JPATH_LIBRARIES_PWEB.'/joomla/http/factory.php', true);
		JLoader::load('JHttpFactory');
	}
...
class JOAuth2Client
{
	...
	public function __construct(JRegistry $options = null, JHttp $http = null, JInput $input = null, JApplicationWeb $application = null)
	{
		...
		$this->http = isset($http) ? $http : new JHttp($this->options, JHttpFactory::getAvailableDriver($this->options)); //PWeb load avialable drivers
