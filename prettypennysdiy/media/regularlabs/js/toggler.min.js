...
(function($){$(document).ready(function(){if($(".rl_toggler").length)RegularLabsToggler.initialize();else $(function(){if($(".rl_toggler").length)RegularLabsToggler.initialize()}).delay(2E3)});RegularLabsToggler={togglers:{},elements:{},initialize:function(){this.togglers=$(".rl_toggler");if(!this.togglers.length)return;this.initTogglers()},initTogglers:function(){var self=this;var new_togglers={};$.each(this.togglers,function(i,toggler){if(toggler.id){$(toggler).show();$(toggler).removeAttr("height");
...
"$1").replace(/^(.*?)\[(.*?)\]/g,"$1_$2").trim();if(el_name!=="")if(typeof self.elements[el_name]!="undefined"){self.elements[el_name].elements.push(el);self.setValues(el_name);self.setElementEvents(el,el_name)}})},setElementEvents:function(el,el_name){var self=this;var type;if(typeof el.type=="undefined"){if($(el).prop("tagName").toLowerCase()=="select")type="select"}else type=el.type;var func=function(){self.toggle(el_name)};switch(type){case "radio":case "checkbox":$(el).bind("click",func).bind("keyup",
func);break;case "select":case "select-one":case "text":$(el).bind("change",func).bind("keyup",func);break;default:$(el).bind("change",func);break}}}})(jQuery);