...
//				if ($type == 'billtoemail') return $row['user_email'];
	...
	function _categoryInfoForProductId( $id ) {
		...
		$query = "SELECT distinct
			...
			left outer join #__virtuemart_product_categories pcx on (vp.`product_parent_id` != 0 AND vp.`product_parent_id` = pcx.`virtuemart_product_id` OR vp.`virtuemart_product_id` = pcx.`virtuemart_product_id` )
