...
class modMetaModHelper
{
	...
	public static function versionMinor() { return '27'; }
...
// FIXME - this is broken in J3.0.
			...
			foreach( array_keys($gets) as $get ) {
				...
				$test = print_r( @$_POST[$get], true );
				if (strpos($test, 'stdClass Object') !== false) continue;
				
				$test = print_r( @$_GET[$get], true );
				if (strpos($test, 'stdClass Object') !== false) continue;
...
//				$val = str_replace("'", '\\\'', $val); // escape single quotes
	...
	public static function getUserIP( $params, $core_genius = null ) {
		...
		return $core_genius->info('client_ip');
	}
	...
	public static function moduleIdsAndChanges( &$params, $my_module = null )
	{
		...
		// set up some JomGeniuses for use in the script and elsewhere
		include_once dirname(__FILE__) . '/JomGenius.class.php';
		...
		$ip = modMetaModHelper::getUserIP( $params, $core_genius );
