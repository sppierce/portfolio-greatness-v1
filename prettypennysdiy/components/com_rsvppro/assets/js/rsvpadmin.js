...
jQuery(document).ready(function(){
    jQuery("#jevattend_email").on('keyup', function() {
        $rtp=jQuery("#registration-tab-pane");
        if ($rtp.hasClass("existing0")) {
            $rtp.removeClass("existing0");
        }
    });
    jQuery("#jevattend_email").on('change', function() {
        $rtp=jQuery("#registration-tab-pane");
        if ($rtp.hasClass("existing0")) {
            $rtp.removeClass("existing0");
        }
...
jQuery(window).on('load', function(){
    jQuery("#updateattendance .tt-menu").on('mousedown', function() {
        $rtp=jQuery("#registration-tab-pane");
        if ($rtp.hasClass("existing0")) {
            $rtp.removeClass("existing0");
        }
...
jQuery(document).ready(function(){
	jQuery('#jevattendsubmit').attr('disabled', true);

	jQuery(':text').keyup(function() {
		if (jQuery('#ta_userid').val() != "" || jQuery('#jevattend_email').val() != "") {
			jQuery('#jevattendsubmit').attr('disabled', false);
