...
class FrontAttendeesController extends AdminAttendeesController
{
	...
	public function recordAttendance()
	{
		...
		if ($user->id == 0 && $this->params->get("attendemails", 0) && $this->params->get("autoregister", 0))
		{
			$user->newid  = $this->createUser($row, $rsvpdata);
                        // useful place for holding this information that can then be used in plugins
                        $rsvpdata->newid = $user->newid;
		...
		if ($jevattend_hidden)
		{
						...
						if ($testattendee)
						{
							...
							if ($this->params->get("allowunconfirmedpendingunpaidchanges",0) && !$testattendee->confirmed && (
								...
								$db->execute();
                                                            ...
                                                            $Itemid = JRequest::getInt("Itemid");
                                                            list($year, $month, $day) = JEVHelper::getYMD();
                                                            $link = JRoute::_($row->viewDetailLink($year, $month, $day, false, $Itemid), false);
                                                            $this->notifyUser($rsvpdata, $row, $user, $testattendee->email_address, $testattendee->email_address, $testattendee,'ack', $testattendee->waiting);
                                                            $mainframe->enqueueMessage(JText::_("JEV_ATTENDANCE_CANNOT_REGISTER_A_SECOND_TIME_USING_THE_SAME_EMAIL_ADDRESS"),"warning");
			...
			if ($rsvpparams ) {
				...
				foreach ($rsvpparamsarray as $pield => $pm)
				{
					...
					if (($pm["type"] == "jevrecaptcha" || $pm["type"] == "jevrecaptcha_v2" ) && $jevattend)
					{
						...
						// Version 1 API
						if (!JComponentHelper::getParams('com_rsvppro')->get("recaptchav2", 1)){
					...
					if ($pm["type"] == "jevrcoupon"){
						...
						if (isset($pm["maxuses"]) && $pm["maxuses"] > 0)
						{
							...
							if ($couponusage){
								...
								if (isset($couponparams->$fieldname) && isset($couponparams->$fieldname->$couponcode) && intval($couponparams->$fieldname->$couponcode)+1 > $maxuses ){
									$canusecoupon = false;
			...
			if (!$attendee)
			{
				...
				{
                                        $tempattendee = new stdClass();
                                        $tempattendee->attendstate = -1; 
					$tempattendee->guestcount = 0;
                                        
                                        $this->countAttendeeContributionToAttendance($row, $rsvpdata, $tempattendee);
			...
			else
			{
						...
						|| ($attendee->attendstate == 2 && $jevattend == 1))
				{
					...
					if ($this->params->get("capacity", 0) && $rsvpdata->capacity > 0)
					{
						...
						{                                                       
                                                        // 10. Existing attendee record was no - now yes
                                                        if ($attendee->attendstate == 0 && $jevattend == 1)
                                                        {
							  // DO WE WANT TO PUT SOMEONE ONTO THE WAITING LIST??
							  $onWaitingList = 1;
                                                          $mainframe->enqueueMessage(JText::_("JEV_ATTENDANCE_ON_WAITING_LIST"));
                                                        }
                                                        else {
                                                            $mainframe->redirect($link, JText::_("JEV_WILL_PUT_YOU_ON_WAITING_LIST"));
                                                        }                                                        
	...
	{
                // TAKE CARE $attendee could be a stdClass!
		...
		if ($attendee->attendstate == 1 || ($attendee->attendstate == 4 && $unpaidtoo))
		{
			...
			if ($rsvpdata->template != "")
			{
				...
				if (is_int($xmlfile) || file_exists($xmlfile))
				{
					...
					{
						if (isset($param ["includeintotalcapacity"]) && $param ["includeintotalcapacity"] && isset($param ["nativevalue"]))
						{
                                                        // multiple attendees 
                                                        if (is_array($param ["nativevalue"])){
                                                            for($g=0;$g<=$row->newcnt; $g++){
                                                                $row->currentcnt += intval($param ["nativevalue"][$g]);
                                                                if (isset($param ["reducevaluefortotalcapacity"]) && intval($param ["nativevalue"][$g]) > 0 && !isset($row->_reductionapplied))
                                                                {
                                                            ...
                                                            $row->currentcnt += intval($param ["nativevalue"]);
                                                            if (isset($param ["reducevaluefortotalcapacity"]) && intval($param ["nativevalue"]) > 0 && !isset($row->_reductionapplied))
                                                            {
                                                                    $row->currentcnt -= $param ["reducevaluefortotalcapacity"];
                                                                    $row->_reductionapplied = 1;
                                                            }
				...
				if (is_int($xmlfile) || file_exists($xmlfile))
				{
					...
					foreach ($params as $param)
					{
						if (isset($param ["includeintotalcapacity"]) && $param ["includeintotalcapacity"] && isset($param ["nativevalue"]))
						{
                                                        ...
                                                        if (is_array($param ["nativevalue"])){
                                                            for($g=0;$g<=$row->newcnt; $g++){
                                                                $row->newcnt += intval($param ["nativevalue"][$g]);
                                                                if (isset($param ["reducevaluefortotalcapacity"]) && intval($param ["nativevalue"][$g]) > 0 && !isset($row->_reductionapplied))
                                                                {
                                                                        $row->newcnt -= $param ["reducevaluefortotalcapacity"];
                                                                        $row->_reductionapplied = 1;
                                                                }
                                                            }
                                                        }
                                                        else {
                                                            $row->newcnt += intval($param ["nativevalue"]);
                                                            if (isset($param ["reducevaluefortotalcapacity"]) && intval($param ["nativevalue"]) > 0 && !isset($row->_reductionapplied))
                                                            {
                                                                    $row->newcnt -= $param ["reducevaluefortotalcapacity"];
                                                                    $row->_reductionapplied = 1;
