...
class FrontSessionsController extends AdminSessionsController   {
	...
	function overview( )
	{
		...
		$whichsessions = $rsvpparams->get("whichsessions","SBD");
		...
		$newcats = $rsvpparams->get( "catidnew", false);
		...
		if ($newcats && is_array($newcats) && count($newcats) > 0){
			if ($jevparams->get("multicategory",0)){
				$where[] = "catmap.catid in ('".implode(",",$newcats)."')";
			}
			else {
				$where[] = "ev.catid in ('".implode(",",$newcats)."')";
			}
		}
		...
		else if ( $rsvpparams->get("personallist")==4) {
		...
		if ($rsvpparams->get("personallist") && $user->id>0){
			...
			else if ($rsvpparams->get("personallist")<3) {
				if ($nonrepeating){
		...
		if ($user->get('id') == 0 && !$rsvpparams->get("attendemails")){
			$where[] = " 0";
		}
		...
		if (!JEVHelper::isAdminUser($user)){
			...
			//$where[] = "(atd.showattendees=1 OR ev.created_by=".intval($user->id).")";
                ...
                if ($catid > 0)
                {
                        if ($jevparams->get("multicategory",0) == 1){
