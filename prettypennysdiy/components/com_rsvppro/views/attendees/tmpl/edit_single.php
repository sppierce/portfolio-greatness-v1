	...
	if (is_int($xmlfile) || file_exists($xmlfile))
	{
		...
		if ($params->getNumParams() > 0)
		{
			...
			$initialstate = $this->rsvpdata->initialstate ? 1 : 3;
                        $existingAttendee = $this->attendee->id ? 1 : 0;
                        
			if ($params->isMultiAttendee())
			{
				$html .= '<div id="registration-tab-pane" class="tab-page existing'.$existingAttendee.'">';
				...
				$sessionparams = RsvpHelper::getTemplateParams($this->rsvpdata);
				...
				{
					$addguestyle='';
					$guestcount = (isset($this->attendee->guestcount) ? $this->attendee->guestcount : 1);
					if (($this->rsvpdata->capacity>0 || $sessionparams->get("maxguesttabs",0)>0) &&  $guestcount>=$capacity) {
						$addguestyle=' style="display:none;" ';
					}
					...
					if ($capacity==1){
						...
						$addGuestButton = '<div class="button2-left"  id="addguest" '.$addguestyle.'>
					<div class="blank">
...
if ($hasparams)
{
	...
	{
		$html .='
		<input type="submit" class="jevclickupdate" value="' . JText::_("JEV_CLICK_TO_UPDATE") . '" onclick="submitbutton(\'attendees.save\');return false;" id="jevattendsubmit" />';
	}
	else
	{
		$html .='
		<input type="submit" class="jevclickattend" value="' . JText::_("JEV_CLICK_TO_ATTEND") . '" onclick="submitbutton(\'attendees.save\');return false;"  id="jevattendsubmit" />';
	}
	$html .='
...
else
{
	...
	else
	{
		...
		$html .='
		<input type="submit" class="jevclicksubmit" value="' . JText::_("JEV_CLICK_TO_SUBMIT") . '" onclick="submitbutton(\'attendees.save\');return false;" '.$style.' id="jevattendsubmit" />';
	}
