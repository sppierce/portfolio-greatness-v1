...
JevHtmlBootstrap::popover('.hasjevtip' , array("trigger"=>"hover focus", "placement"=>"top", "container"=>"#jevents", "delay"=> array( "show"=> 150, "hide"=> 150 ), "mouseonly" => true));
...
<div id="jevents" class="rsvppro_sessions">
    ...
    <form action="<?php echo JRoute::_("index.php?option=com_rsvppro&task=sessions.list&Itemid=$Itemid"); ?>"
        ...
        <ul class="rsvpsessionfilters">
			...
			<?php echo $this->hidepast; ?>
                ...
                if (isset($user->id) && $user->authorise('core.create', 'com_jevents') && !$authorisedonly) {
                    ...
                    <li><?php echo JText::_('JEV_ON_OR_AFTER'); ?><br/>                    
                        <?php 
                        $minyear = JEVHelper::getMinYear();
                        $maxyear = JEVHelper::getMaxYear();

                        $inputdateformat = $jevparams->get("com_editdateformat", "d.m.Y");
                        echo JEVHelper::loadElectricCalendar('after', 'after', $this->after, $minyear, $maxyear, '', 'document.adminForm.submit();' , $inputdateformat);
                        ?>
                    </li>                
		 <!--
		...
		//-->                 
                <?php }
                if (isset($this->catlist) && strpos($this->catlist, "<select")!==false) { ?>
                    <li>
                        <?php echo JText::_('JEV_CATEGORIES'); ?><br/>
                        <?php echo isset($this->catlist) ? $this->catlist : ""; ?>
                    </li>
                <?php }  ?>                    
        </ul>
		...
		//-->
			...
			data-content="<?php echo htmlspecialchars($row->repeat->summary, ENT_QUOTES); ?>" >
						<a href="<?php echo $link; ?>">
                            <?php echo $this->escape($row->repeat->summary); ?></a>
					</span>
                    </td>
                    <td align="center">
