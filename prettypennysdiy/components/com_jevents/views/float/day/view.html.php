 ...
 * @static
 */
class FloatViewDay extends JEventsFloatView
{
	...
	function listevents($tpl = null)
	{
		...
		$fallbackview = $jevparams->get("floatfallbackview", "geraint");
		...
		$newViewClass = str_replace("Float", ucfirst($fallbackview), get_class($this));
		include_once($fallbackabstractfile);
		include_once($fallbackfile);
		// TODO Joomla 3.0
		// Use DIRECTORY_SEPARATOR for windows servers !!!
		$name = str_replace(array("float".DIRECTORY_SEPARATOR, "float/"),array($fallbackview.DIRECTORY_SEPARATOR, "$fallbackview/"), $this->_name);
		$theme = $fallbackview;
		$view = str_replace(array("float".DIRECTORY_SEPARATOR, "float/"),"", $this->_name);
		...
		$this->data = isset($newView->data)?$newView->data :  $data;
