...
function FlatplusViewHelperHeader16($view)
{
...
<div id="jevents">
			...
			<ul class="actions">
				...
				if ($cfg->get('com_print_icon_view', 1))
				{
					...
					if ($pop)
					{
						...
						<?php echo JHTML::_('image', 'components/com_jevents/views/flatplus/assets/images/.png', JText::_('JEV_CMN_PRINT'), array("title" => JText::_('JEV_CMN_PRINT')), false); ?>
							</a>
						</li> <?php
					}
					else
				...
				if ($cfg->get('com_email_icon_view', 1))
				{
					...
					?>
					<li class="email-icon">
						<a href="javascript:void(0);" onclick="javascript:window.open('<?php echo $url; ?>', 'emailwin', 'width=400,height=350,menubar=yes,resizable=yes');
	...
	<?php
    if  ((($task == "month.calendar" && !$params->get('fplistmonth', 0)) || ($task == "week.listevents" && $params->get('fptabularweek', 0)) ))
    {
		$baseurl = JURI::root();
		$version = JEventsVersion::getInstance();
