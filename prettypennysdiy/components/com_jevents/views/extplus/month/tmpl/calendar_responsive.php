...
//$this->data["fieldsetText"] = strftime("%B %Y", mktime(0,0,0,1,$this->data["month"], $this->data["year"]));
...
for ($dn = 0; $dn < $datacount; $dn++)
{
	...
	{
		$this->data["dates"][$dn]["eventcount"] = count($this->data["dates"][$dn]["events"]);
							...
							for ($d = 0; $d < 7 && $dn3 < $datacount; $d++)
							{
								...
								if (array_key_exists($slot, $currentDay["slots"]))
								{
									...
									if ($blocks > 0)
									{
										...
										if ($cfg->get("extplusscalabledayname", 1) && !in_array($datestp, $hiddendatearray) && (!isset($lastdatestp) || $datestp != $lastdatestp))
										{
											$lastdatestp = $datestp;
											$hiddendatearray[] = $datestp;
											?>
											<div class="hiddendayname jev_daysnames jev_daysnames_<?php echo $this->colourscheme; ?> jev_<?php echo $this->colourscheme; ?> jev_none">
												<span>
													<?php
