...
{
	JEVHelper::script('components/' . JEV_COM_COMPONENT . '/assets/js/editpopupJQ.js');
	$this->popup = true;
							...
							if (array_key_exists($slot, $currentDay["slots"]))
							{
								...
								if ($blocks > 0)
								{
									...
									$datestp = JevDate::mktime(0, 0, 0, $event->mup(), $event->dup(), $event->yup());
                                                                        $day_link = "<a href='" . $currentDay['link'] . "'>" . $event->starttime() . "</a>";

                                                                        if ($event->allDayEvent()){
                                                                                $day_link = "<a href='" . $currentDay['link'] . "'  style='visibility:hidden'>" . $event->starttime() . "</a>";
                                                                        }
                                                                        else{
                                                                                $day_link = "<a href='" . $currentDay['link'] . "'>" . $event->starttime() . "</a>";
                                                                        }
									...
									// For multi-day events we much add a few of these
									/*
									if ($blocks > 1)
									{
										for($b=1;$b<$blocks; $b++){
											$datestp = JevDate::mktime(0, 0, 0,$event->mup(), $event->dup()+$b, $event->yup());
											$day_link = "<a href='" . $currentDay['link'] . "'>" . JString::substr($event->_startrepeat, 11, 5) . "</a>";
											echo "<span class='hiddentime hiddentime$b'>$day_link</span>";
										}
									}
									*/

									if ($cfg->get("ruthinscalabledayname", 1) && !in_array($datestp, $hiddendatearray) && (!isset($lastdatestp) || $datestp != $lastdatestp))
									{
										$lastdatestp = $datestp;
										...
										<div class="hiddendayname jev_daysnames jev_daysnames_<?php echo $this->colourscheme; ?> jev_<?php echo $this->colourscheme; ?>" style="display:none">
											<span>
												...
												{
													$hiddendayname = JString::substr($this->data["daynames"][$weekday], strpos($this->data["daynames"][$weekday], ">") + 1, strrpos($this->data["daynames"][$weekday], "<") - strpos($this->data["daynames"][$weekday], ">") - 1) . " " . JevDate::strftime("%d", $currentDay["cellDate"]);
												}
												else
